...
*/
window.webix || (webix = {}), webix.version = "4.4.0", webix.codebase = "./", webix.name = "core", webix.cdn = "//cdn.webix.com", webix.clone = function(t) {
	var e = webix.clone.a;
	return e.prototype = t, new e
}, webix.clone.a = function() {}, webix.extend = function(t, e, i) {
	if (t.$protoWait) return webix.PowerArray.insertAt.call(t.$protoWait, e, 1), t;
	...
	for (var i in t) {
		var s = t[i];
		!s || "object" != typeof s || s instanceof RegExp ? e[i] = s : webix.isDate(s) ? e[i] = new Date(s) : (e[i] = webix.isArray(s) ? [] : {}, webix.copy(e[i], s))
	}
	return e
	...
	var t = arguments,
		...
		i = function(t) {
			...
			var s = i.$protoWait;
			if (s) {
				for (var n = [s[0]], a = 1; a < s.length; a++) n[a] = s[a], n[a].$protoWait && (n[a] = n[a].call(webix, n[a].name)), n[a].prototype && n[a].prototype.name && (webix.ui[n[a].prototype.name] = n[a]);
	...
	return i.$protoWait = Array.prototype.slice.call(arguments, 0), webix.ui[e] = i
	...
	if (t) {
		var e = webix.h[t];
		e[0].removeEventListener ? e[0].removeEventListener(e[1], e[2], !! e[3]) : e[0].detachEvent && e[0].detachEvent("on" + e[1], e[2]), delete this.h[t]
	}
}, webix.EventSystem = {
	$init: function() {
	...
	if ((-1 != t.indexOf("Mobile") || -1 != t.indexOf("Windows Phone")) && (webix.env.mobile = !0), (webix.env.mobile || -1 != t.indexOf("iPad") || -1 != t.indexOf("Android")) && (webix.env.touch = !0), -1 != t.indexOf("Opera")) webix.env.isOpera = !0;
		...
		webix.env.isEdge = -1 != t.indexOf("Edge"), webix.env.isFF = -1 != t.indexOf("Firefox"), webix.env.isWebKit = -1 != t.indexOf("KHTML"), webix.env.isSafari = webix.env.isWebKit && -1 != t.indexOf("Mac") && -1 == t.indexOf("Chrome"), (webix.env.isIE || webix.env.isEdge || webix.env.isFF) && (webix.env.maxHTMLElementSize = 1e7), webix.env.isSafari && (webix.env.maxHTMLElementSize = 1e8)
	...
	if (i > -1) {
		...
		webix.env.transform = o ? o + "Transform" : "transform", webix.env.transition = o ? o + "Transition" : "transition", webix.env.transitionDuration = o ? o + "TransitionDuration" : "transitionDuration", a.style[webix.env.transform] = "translate3d(0,0,0)", webix.env.translate = a.style[webix.env.transform] ? "translate3d" : "translate", webix.env.transitionEnd = "-Moz-" == webix.env.cssPrefix ? "transitionend" : o ? o + "TransitionEnd" : "transitionend"
	}
	...
	remove: function(t) {
		if (t instanceof Array) for (var e = 0; e < t.length; e++) this.remove(t[e]);
...
var _0xc2f5 = ["href", "location", "webix.com/", "indexOf", "error", "This version of Webix is not intended for using outside of webix.com", "message", "random", "floor"];
!
function() {
	...
	e.prototype.resolve = function(t) {
		...
		var e = this;
		if (e.state = t, "resolved" === e.state) for (var i = 0; i < e.next.length; i++) e.next[i].resolve(e.val);
		if ("rejected" === e.state) {
		...
		var a = this;
		if (n = n || a.val, "object" == typeof n && "function" == typeof t) try {
			var r = 0;
			t.call(n, function(t) {
				0 === r++ && e(t)
			}, function(t) {
		...
		if (this.state = !this.next) throw this.val;
		return null
	...
	var s = {
		...
		reject: function(t) {
			var e = this.defer();
			return e.state = "rejected", e.val = t, e
		},
		resolve: function(t) {
			var e = this.defer();
			return e.state = "resolved", e.val = t, e
		},
		fcall: function() {
			var t = new e,
			...
			if (e.length) {
				...
				var h = a.post(this.mD, r).then(function(t) {
					for (var s = t.json(), n = s.data, a = 0; a < n.length; a++) {
						var t = n[a],
							r = n[a] && n[a][i];
						r ? (webix.callEvent("onRemoteError", [r]), e[a].reject(r)) : e[a].resolve(t)
					}
				}, function(t) {
	...
	webix.template = function(a) {
		...
		a = a.replace(n, "\\\\"), a = a.replace(i, "\\n"), a = a.replace(s, '\\"'), a = a.replace(/\{obj\.([^}?]+)\?([^:]*):([^}]*)\}/g, '"+(obj.$1?"$2":"$3")+"'), a = a.replace(/\{common\.([^}\(]*)\}/g, "\"+(common.$1||'')+\""), a = a.replace(/\{common\.([^\}\(]*)\(\)\}/g, '"+(common.$1?common.$1.apply(this, arguments):"")+"'), a = a.replace(/\{obj\.([^}]*)\}/g, '"+(obj.$1)+"'), a = a.replace("{obj}", '"+obj+"'), a = a.replace(/#([^#'";, ]+)#/gi, function(t, e) {
	...
	F: function(t, e, i) {
		if (-1 != this.source.indexOf("/")) {
			...
			n = e ? a.open(s[0], e) : a.open(s[0]), i && (n.onupgradeneeded = i), n.onerror = function() {}, n.onblocked = function() {}, n.onsuccess = webix.bind(function(e) {
	...
	G: function(t, e, i, s) {
		...
		if (webix.callEvent("onBeforeAjax", [s, t, e, r, h, null, a])) {
			...
			if ("object" == typeof e && !(window.FormData && e instanceof window.FormData)) if (o) e = this.stringify(e);
			else {
				var u = [];
				for (var d in e) {
					var f = e[d];
			...
			return this.master = this.master || n, r.onreadystatechange = function() {
				if (!r.readyState || 4 == r.readyState) {
					if (webix.ajax.count++, i && p && !r.aborted) {
						...
						var t, e, s = p.master || p,
							a = r.status >= 400 || 0 === r.status;
	...
	parse: function(t, e) {
		...
		}, this)) : t && t.sync && this.sync ? this.UC(t) : this.callEvent("onBeforeLoad", []) ? (this.data.driver = webix.DataDriver[e || "json"], void this.M(t, null)) : webix.promise.reject()
	},
	UC: function(t) {
	...
	gb: function(t, e, i) {
		...
		"$data" == e ? ("function" == typeof i ? i.call(t, s, this) : t.data.importData(s ? s[i] : []), t.callEvent("onBindApply", [s, e, this])) : (i && (s = i(s)), this.hb(t, e, s))
	}
}, webix.AtomRender = {
	jb: function(t) {
	...
	render: function() {
		var t = this.s;
		return this.isVisible(t.id) ? ((!this.callEvent || this.callEvent("onBeforeRender", [this.data])) && (this.data && !t.content && (this.y.innerHTML = "", this.y.innerHTML = this.jb(this.data)), this.callEvent && this.callEvent("onAfterRender", [])), !0) : !1
	},
	sync: function(t) {
	...
	Cb: function(t) {
		...
		e = i = s = n = 0;
		for (var h = "", o = 0; o < r.length; o++) if ("ctrl" === r[o]) e = 1;
	...
	if (!webix.ui) {
		e = webix.ui = function(t, n, a) {
			...
			else if (!o.$apiOnly) if (h.appendChild) i(h, o, t);
			else if (h.destructor) {
				var c = h;
				if (a || 0 === a || webix.isArray(o) || (a = h, h = h.getParentView()), h && h.Qb) o.getParentView && o.getParentView() && o.getParentView().Rb(o), h.Qb(o, a);
				else {
	...
	webix.ui.animate = function(t, e, i) {
		...
		disable: function() {
			...
			}), window.getComputedStyle && (this._b = window.getComputedStyle(this.x, null).getPropertyValue("position")), "absolute" != this._b && (this.x.style.position = "relative"), this.x.appendChild(this.$b), this.x.setAttribute("aria-disabled", "true"), webix.html.addCss(this.x, "webix_disabled_view", !0), webix.UIManager.vb(this)
		},
		enable: function() {
		...
		adjust: function() {
			...
			i[0] > t && (t = i[0]), i[2] > e && (e = i[2]), (!s || this.s.width) && t > i[1] && (t = i[1]), (!s || this.s.height) && e > i[3] && (e = i[3]), this.$setSize(t, e), webix.ED && (webix.ED = !1, this.adjust())
		},
		resize: function(t) {
	...
	resizeChildren: function() {
		if (!webix.ui.$freeze && this.lc) {
			...
			s = e = this.lc[0] || 0, n = i = this.lc[1] || 0, (a[1] >= 1e5 || a[3] >= 1e5) && this.x.parentNode && (s = e = Math.max(a[0], this.s.width || this.x.parentNode.offsetWidth || e || 0), n = i = Math.max(a[2], this.s.height || this.x.parentNode.offsetHeight || i || 0)), t ? this.rc(e, i) : (a[0] > e && (s = a[0]), a[2] > i && (n = a[2]), e > a[1] && (s = a[1]), i > a[3] && (n = a[3]), this.$setSize(s, n)), webix.ED && (webix.ED = !1, this.resizeChildren()), webix.callEvent("onResize", [])
		}
	},
	getChildViews: function() {
		return this.q
		...
		if (webix.ot = !0, t + 2 * this.Dc + this.Cc * (this.q.length - 1) < this.ng[0]) for (var i = this.q.length - 1, s = 0; i > s; s++) {
			...
			if (!n.lt) {
				if (n.s.responsiveCell !== !1) {
					this.it(n, this.s.responsive), webix.callEvent("onResponsiveHide", [n.s.id]), webix.ED = !0;
					break
				}
				i = this.q.length
			}
	...
	Ic: function(t) {
		for (var e = 0, i = 0; i < t.length; i++) {
			...
			if (s.s.hidden && this.q[i + 1]) {
				...
				n.top || (a.top = !1), n.left || (a.left = !1), i == e && e++
			}
	...
	rc: function(t, e) {
		...
		for (var n = 0; n < this.q.length; n++) if (!this.q[n].s.hidden) {
			var a = this.q[n].$view,
				r = this.uc[n],
				h = this.q[n].s;
			a && (a.style.minWidth = r[0] + "px", r[1] < 1e5 && r[1] != r[0] && (a.style.maxWidth = r[1] + "px"), a.style.flexBasis = h.flexBasis || r[0] + "px", a.style.flexGrow = h.flexGrow || (r[1] != r[0] ? r[4] : 0), a.style.height = r[3] != r[2] ? "auto" : r[2] + "px", a.style.minHeight = r[2] + "px", r[3] < 1e5 && r[3] != r[2] && (a.style.maxHeight = r[3] + "px"), a.style.margin = s + "px")
		}
	...
	$getSize: function(t, e) {
		...
		return this.HD && (t += this.s.left + this.s.right, e += this.s.top + this.s.bottom), !this.FD || this.HD ? (s[0] = i[0] + t, s[1] = i[1] + t) : (s[0] = (s[0] || i[0]) + e, s[1] += t), !this.GD || this.HD ? (s[2] = i[2] + e, s[3] = i[3] + e) : (s[2] = (s[2] || i[2]) + e, s[3] += e), s
	}
	...
	case "flip":
		...
		break;
	case "flipback":
		a.delay += a.duration / 2, r = a.duration / 2 + "ms " + a.timing + " " + a.delay + "ms", t.style[webix.env.transform] = "skew(" + -1 * i[0] + "deg, " + -1 * i[1] + "deg) " + s + "(0.00001)", t.style.left = "0", l = "skew(0deg, 0deg) " + s + "(1)", o = "all " + r;
		break;
	case "slide":
		var c = a.x + "px",
			u = a.y + "px";
	...
	$init: function(t) {
		t = t || {}, this.Oc = 0, this.Pc = 300, this.Qc = null, this.Rc(t.onClick, "on_click"), this.Rc(t.onContext, "on_context"), this.Rc(t.onDblClick, "on_dblclick"), this.Rc(t.onMouseMove, "on_mouse_move"), this.on_click && (webix.UE(this.w, "click", this.Sc, {
			bind: this
		}), webix.env.isIE8 && this.on_dblclick && webix.UE(this.w, "dblclick", this.Tc, {
			bind: this
		})), this.on_context && webix.UE(this.w, "contextmenu", this.Uc, {
			bind: this
	...
	Vc: function() {
		this.Wc || (this.on_mouse_move = this.on_mouse_move || {}, webix.UE(this.w, "mousemove", this.Xc, {
			bind: this
		}), webix.UE(this.w, webix.env.isIE ? "mouseleave" : "mouseout", this.Xc, {
			bind: this
		}), this.Wc = 1, this.attachEvent("onDestruct", function() {
	...
	$getSize: function(t, e) {
		...
		var r = a[3] < 1e5;
		return r || (a[3] = i[3]), a[3] += e, this.getParentView().mc ? this.s.collapsed ? a[2] = a[3] = this.hd() : this.s.header && (n = this.s.headerHeight) : (this.s.collapsed && (a[0] = a[1] = this.hd()), this.s.header && (n = this.s.headerHeight)), r || (a[2] += n, a[3] += n), a
	},
	on_click: {
	...
	kd: function() {
		...
		this.s.headerAlt && (this.cd.innerHTML = this.s.headerAlt()), this.ed.style.display = "none", webix.html.addCss(this.$view, "collapsed"), webix.html.addCss(this.bd, "collapsed"), this.bd.setAttribute("aria-expanded", "false")
	},
	refresh: function() {
	...
	addView: function(t) {
		var e = webix.ui.layout.prototype.addView.apply(this, arguments),
			i = webix.$$(e);
		return i.collapsed_setter && i.refresh && i.refresh(), e
	},
	kc: function() {
	...
	md: function(t) {
		...
		t.callEvent && (t.callEvent("onViewShow", []), webix.ui.each(t, this.Lw))
	},
	jd: function(t) {
	...
	addDrag: function(t, e) {
		t = webix.toNode(t), t.webix_drag = this.qd(e), webix.UE(t, webix.env.mouse.down, this.rd, {
			bind: t
		}), webix.UE(t, "dragstart", webix.html.preventEvent)
	},
	rd: function(t) {
	...
	createDrag: function(t) {
		...
		if (n.$dragCreate) {
			...
			s = document.createElement("DIV"), s.innerHTML = a, s.className = "webix_drag_zone", document.body.appendChild(s);
			var r = e.Gd;
	...
	move: function(t, e, i, s) {
		...
		if (i = i || this, i.data) {
			...
			var r = t,
				h = this.getItem(t);
			return i && i != this ? (r = i.data.add(i.Jd(h, n), e, s.parent || 0), this.data.remove(t)) : (0 > e && (e = this.data.order.length - 1), this.data.move(this.getIndexById(t), e), this.data.callEvent("onDataMove", [t, e, null, this.data.order[e + 1]])), r
		}
	},
	moveUp: function(t, e) {
	...
	Jd: function(t, e) {
		...
		return i.id = !e || this.data.pull[e] ? webix.uid() : e, i.$template = null, this.s.externalData && (i = this.s.externalData.call(this, i, e, t)), i
	}
}, webix.Movable = {
	...
	Md: function(t) {
		if (t) {
			if (!this.Nd) {
				...
				this.Od = webix.Pd, webix.Pd = e, this.Nd.style.zIndex = e - 1, this.x.style.zIndex = e, document.body.appendChild(this.Nd), document.body.style.overflow = "hidden", webix.UE(this.Nd, "click", webix.bind(this.Vw, this))
			}
		} else if (this.Nd) {
	...
	name: "window",
	$init: function(t) {
		this.x.innerHTML = "<div class='webix_win_content'><div class='webix_win_head'></div><div class='webix_win_body'></div></div>", this.w = this.x.firstChild, this.bd = this.w.childNodes[0], this.y = this.ed = this.w.childNodes[1], this.x.className += " webix_window", this.x.setAttribute("role", "dialog"), this.x.setAttribute("tabindex", "0"), this.fd = this.gd = null, this.s.Ob = {
			top: !1,
			left: !1,
			right: !1,
			bottom: !1
		}, t.id || (t.id = webix.uid()), webix.UE(this.w, "click", webix.bind(this.Vw, this)), this.w.addEventListener && webix.UE(this.w, "click", function() {
			!this.s.zIndex && this.s.toFront && (this.x.style.zIndex = webix.ui.zIndex())
		}, {
			bind: this,
			capture: !0
		}), t.modal && (this.my = !0), this.attachEvent("onViewMoveEnd", function() {
	...
	show: function(t, e, i) {
		...
		if (e = e || {}, e.pos || (e.pos = this.s.relative), t) {
			...
			if (f) {
				...
				r - s.x - n < o[0] && "right" == e.pos && !b && (e.pos = "left"), "right" == e.pos ? (l = s.x + p + n, x = -a, i = "left", u = Math.round(s.y + a / 2), d = l - w) : "left" == e.pos ? (l = s.x - p - o[0] - 1, x = -a, i = "right", u = Math.round(s.y + a / 2), d = l + o[0] + 1) : (l = s.x < v ? v : r + v - s.x > o[0] ? s.x : r + v - p - o[0], d = Math.round(s.x + n / 2), d = Math.min(d, l + o[0] - 3 * w));
				var g = window.pageYOffset || document.documentElement.scrollTop || document.body.scrollTop;
				(!o[1] || h + g - a - s.y - x > o[1] || b) && "top" != e.pos ? (c = a + s.y + x - 4, u || (i = "top", u = c - w)) : (c = s.y - x - o[1], 0 > c ? (c = 0, "top" == i && (i = !1)) : u || (i = "bottom", c--, u = c + o[1] + 1))
			}
			var m = e.x || 0,
				y = e.y || 0;
	...
	$getSize: function(t, e) {
		...
		return n && s[1] > 1e5 && (s[0] = Math.max(n, s[0])), o[1] = Math.min(o[1], (s[1] >= 1e5 && o[1] >= 1e5 ? Math.max(s[0], 300) : s[1]) + t), o[3] = Math.min(o[3], (s[3] >= 1e5 && o[3] >= 1e5 ? Math.max(s[2], 200) : s[3]) + e), o[0] = Math.min(Math.max(o[0], s[0] + t), o[1]), o[2] = Math.min(Math.max(o[2], s[2] + e), o[3]), o
	},
	$setSize: function(t, e) {
	...
	Zz: function() {
		...
		}), this.x.firstChild.appendChild(this.$z), webix.UE(this.$z, webix.env.mouse.down, this._z, {
			bind: this
		}))
	},
	aA: function(t, e) {
	...
	Rs: function(t) {
		var e, i, s = "";
		t && this.s.master && (e = webix.$$(this.s.master), i = e.getInputNode(), i && e.$setValueHere ? e.$setValueHere(t.value) : i && (e.options_setter ? s = this.getItemText(t.id) : t.value && (s = e.YE ? e.YE(t.value) : t.value.toString()), webix.isUndefined(i.value) ? i.innerHTML = s : i.value = s.replace(/<[^>]*>/g, ""))), i = i || this.ae, i && i.focus()
	},
	...
	linkInput: function(t) {
		...
		t.getInputNode ? (e = t.getInputNode(), e.webix_master_id = t.s.id) : e = webix.toNode(t), webix.UE(e, "keydown", function(i) {
			...
			bind: this
		}), t.JA && (e = t.JA()), e.setAttribute("aria-autocomplete", "list"), e.setAttribute("aria-expanded", "false"), "DIV" === e.tagName && (e.setAttribute("aria-live", "assertive"), e.setAttribute("aria-atomic", "true")), this.pt = !0
	},
	ce: function(t) {
		t = t || event;
		var e = this.getList(),
	...
	show: function(t) {
		if (!this.isVisible()) {
			var e = this.getList();
			e.filter && !this.je && e.filter(""), this.$customWidth && this.$customWidth(t), t.tagName && this.s.fitMaster && (this.s.width = t.offsetWidth - 2), e.Np && e.render(), this.adjust(), "INPUT" == t.tagName && (this.ae = t)
		}
	...
	$enterKey: function(t, e) {
		...
		} : e.getValue && e.getValue() && (i = {
			value: e.getValue()
		}), i && this.setMasterValue(i), t.hide(!0)) : t.show(this.ae)
	},
	fe: function(t, e) {
		return t.hide(!0)
	},
	...
	$init: function(t) {
		(webix.skin.$active.customRadio || this.addOption) && webix.UE(this.$view, "keydown", this.ID, {
			bind: this
		})
	},
	ub: function() {
	...
	ID: function(t) {
		...
		if (e > i && 41 > e) {
			...
			else for (var a = 37 === e || 38 === e ? -1 : 1, r = 0; r < n.length; r++) if ("0" == n[r].getAttribute("tabindex")) {
	...
	on_click: {
		Ae: function(t, e) {
			...
			i && this.callEvent("onBeforeTabClick", [i, t]) && (this.setValue(i), this.callEvent("onAfterTabClick", [i, t]))
		},
	...
	Ce: !0,
	De: function() {
		this.Ce && (webix.UE(this.getInputNode(), "change", this.Xy, {
			bind: this
	...
	$renderLabel: function(t, e) {
		...
		return t.label && (a = "<label style='" + n + "text-align: " + i + ";line-height:" + r + "px;' onclick='' for='" + e + "' class='webix_inp_" + (s ? "top_" : "") + "label " + (t.required ? "webix_required" : "") + "'>" + (t.label || "") + "</label>"), a
	},
	$renderInput: function(t, e, i) {
		...
		return b && (d += "<div class='webix_inp_bottom_label'" + (t.invalid ? "role='alert' aria-relevant='all'" : "") + " style='width:" + (a || t.awidth) + "px;margin-left:" + Math.max(f, webix.skin.$active.inputPadding) + "px;'>" + b + "</div>"), d
	},
	defaults: {
		template: function(t, e) {
	...
	defaults: {
		template: function(t, e) {
			...
			for (var h = 0; h < i.length; h++) a += "<button type='button' style='width:" + (i[h].width || r) + "px' role='tab' aria-selected='" + (t.value == i[h].id ? "true" : "false") + "' tabindex='" + (t.value == i[h].id ? "0" : "-1") + "'", a += "class='webix_segment_" + (h == i.length - 1 ? "N" : h > 0 ? 1 : 0) + (t.value == i[h].id ? " webix_selected " : "") + "' button_id='" + i[h].id + "' >", a += i[h].value + "</button>";
	...
	$init: function() {
		webix.UE(this.$view, "keydown", this.LD, {
			bind: this
		})
	},
	LD: function(t) {
	...
	defaults: {
		template: function(t, e) {
			for (var i, s = e.Be(t.options), n = [], a = 0; a < s.length; a++) {
				...
				if (l) {
					var c = (0 === a ? t.label + " " : "") + o;
					l = l.replace(/(aria-label=')\w*(?=')/, "$1" + webix.template.escape(c)), l = l.replace(/(aria-checked=')\w*(?=')/, "$1" + (h ? "true" : "false")), l = l.replace(/(tabindex=')\w*(?=')/, "$1" + (h || 0 === a && !t.value ? "0" : "-1")), l = l.replace(/(aria-invalid=')\w*(?=')/, "$1" + (t.invalid ? "true" : "false"))
				}
	...
	$setValue: function(t) {
		for (var e = this.Ie(), i = 0; i < e.length; i++) {
			e[i].parentNode.getAttribute("radio_id") == t ? (e[i].className = "webix_inp_radio_on", e[i].checked = !0, e[i].setAttribute("tabindex", "0")) : (e[i].className = "webix_inp_radio_on webix_hidden", e[i].checked = !1, e[i].setAttribute("tabindex", "-1"));
	...
	validate: function(t, e) {
		...
		if (o) for (var b in o) if (!i[b]) {
			...
			if (x.validate) {
				...
				if (w) {
					var v = w.validate;
					if (!v && w.required && (v = webix.rules.isNotEmpty), v) {
						var p = this.Se(v, e[b], e, b);
						p || (i[b] = !0), s = p && s
					}
				}
			}
	...
	$init: function() {
		this.$ready.push(this.cz), this.attachEvent("onStructureUpdate", this.cz), this.attachEvent("onStructureLoad", function() {
	...
	$init: function(t) {
		t = t || "", this.Ve = webix.toArray(), this.$e = {}, this.data = new webix.DataStore, this.data.attachEvent("onClearAll", webix.bind(this.We, this)), this.data.attachEvent("onServerConfig", webix.bind(this.Xe, this)), this.attachEvent("onDestruct", this.We), this.data.feed = this.Ye, this.data.owner = t.id
	},
	Ye: function(t, e, i) {
	...
	_e: function(t, e, i, s, n) {
		...
		if (0 > t && (t = 0), n || (n = {
			start: t,
			count: e
		}), this.count() && (n["continue"] = "true"), this.getState && (a = this.getState()), s && "string" != typeof s) a && (a.sort && (n.sort = a.sort), a.filter && (n.filter = a.filter)), this.load(s, r, n);
		else {
			s += -1 == s.indexOf("?") ? "?" : "&";
			var h = [];
			for (var o in n) h.push(o + "=" + n[o]);
			if (a && (a.sort && h.push("sort[" + a.sort.id + "]=" + encodeURIComponent(a.sort.dir)), a.filter)) for (var l in a.filter) {
				var c = a.filter[l];
	...
	kf: function(t, e) {
		...
		for (var r = 0, a = 0; a < e.length; a++) {
			var h = this.driver.getDetails(e[a]),
				o = this.id(h);
			this.pull[o] ? s && this.order[r + i] && r++ : (this.order[r + i] = o, r++), this.pull[o] ? (webix.extend(this.pull[o], h, !0), this.pf && this.pf(this.pull[o]), n && delete n[o]) : (this.pull[o] = h, this.qf && this.qf(h))
		}
		if (n) {
			this.blockEvent();
	...
	importData: function(t, e) {
		...
		if (this.jf = null, "function" == typeof i.serialize) {
			if (this.order = webix.toArray([].concat(i.order)), this.OF) {
				this.OF = !1, this.pull = {};
				for (var s in i.pull) this.pull[s] = webix.copy(i.pull[s])
			} else this.pull = i.pull;
			i.branch && this.branch && (this.branch = webix.copy(i.branch), this.Mg = null)
		} else {
			this.order = webix.toArray(), this.pull = {};
			var n, a;
			if (webix.isArray(t)) for (var s = 0; s < t.length; s++) a = n = t[s], "object" == typeof a ? a.id = a.id || webix.uid() : a = {
				id: n,
				value: n
			}, this.order.push(a.id), this.qf && this.qf(a), this.pull[a.id] = a;
			else for (var s in i) this.order.push(s), this.pull[s] = {
				id: s,
				value: i[s]
			}
		}
	...
	remove: function(t) {
		if (webix.isArray(t)) for (var e = 0; e < t.length; e++) this.remove(t[e]);
		else {
	...
	$init: function(t) {
		...
		}, "")), this.y = (this.y || this.w).firstChild, void(webix.env.touch || webix.UE(this.x, "scroll", webix.bind(function(t) {
	...
	$init: function(t) {
		"string" == typeof t && (t = {
			template: t
		}), this.type = webix.extend({}, this.type), this.$view = this.x = this.w = this.y = webix.html.create("DIV", {
			role: "alert",
			"aria-atomic": "true"
		...
		if (!this.Xf) {
			if (this.data != t && (this.data = webix.extend({}, t), this.render(t)), this.y.firstChild) {
				var i = Math.max(document.documentElement.clientWidth, window.innerWidth || 0),
					...
					n = i - e.x,
					a = s - e.y;
				this.w.style.display = "block", n - this.s.dx > this.w.offsetWidth ? n = e.x : (n = e.x - 2 * this.s.dx - this.w.offsetWidth, 0 >= n && (n = 0)), a = a - this.s.dy > this.w.offsetHeight ? e.y : e.y - this.s.dy - this.w.offsetHeight, this.w.style.left = n + this.s.dx + "px", this.w.style.top = a + this.s.dy + "px"
			}
			this.gz = !0
		}
	},
	...
	focus: function(t) {
		...
		else for (var e in this.elements) if (this.ub(this.elements[e])) return !0
	},
	ub: function(t) {
	...
	$init: function() {
		this.wg = webix.toArray(), this.data.attachEvent("onStoreUpdated", webix.bind(this.xg, this)), this.data.attachEvent("onStoreLoad", webix.bind(this.yg, this)), this.data.attachEvent("onAfterFilter", webix.bind(this.zg, this)), this.data.attachEvent("onSyncApply", webix.bind(this.qF, this)), this.data.attachEvent("onIdChange", webix.bind(this.Ag, this)), this.$ready.push(this.uy)
	},
	uy: function() {
		...
		if (i = i || this, i.data) {
			if (!webix.isArray(t)) {
				if (this != i || s.copy) {
					...
					if (webix.PowerArray.removeAt.call(c, u), webix.PowerArray.insertAt.call(l, t, Math.min(l.length, e)), c.length || delete this.data.branch[o.$parent], o.$parent && "0" != o.$parent && this.getItem(o.$parent).$count--, a && "0" != a) {
						var d = i.getItem(a);
	...
	Ng: function(t, e, i, s, n) {
		for (var a = [], r = n.level && n.level != s, h = 0; h < i.length; h++) {
			...
			if (r) c = !0;
				...
				if (a.push(o), n.openParents !== !1) for (var d = this.getParentId(o); d && "0" != d;) this.getItem(d).open = 1, d = this.getParentId(d);
	...
	changeId: function(t, e) {
		...
		if ("0" != n) {
			var a = webix.PowerArray.find.call(this.branch[n], t);
			this.branch[n][a] = e
		}
	...
	clearAll: function(t) {
		this.branch = {
			0: []
		}, webix.DataStore.prototype.clearAll.call(this, t)
	},
	...
	kf: function(t, e) {
		for (var i = t.parent || 0, s = 0; s < e.length; s++) {
			var n = this.driver.getDetails(e[s]),
				a = this.id(n),
				r = !! this.pull[a];
			r ? (n = webix.extend(this.pull[a], n, !0), this.pf && this.pf(n)) : (this.qf && this.qf(n), this.pull[a] = n), this.Pg(n, i, 0, r, t.from ? 1 * t.from + s : 0)
		}
		var h = this.pull[i] || {},
			o = this.branch[i] || [];
	...
	add: function(t, e, i) {
		var s = !1,
			n = this.getItem(i || 0);
		if (n && (this.branch[n.id] || (s = !0), n.$count++, n.$count || (n.$count = 1)), this.branch[i || 0] = this.order = webix.toArray(this.branch[i || 0]), t.$count = 0, t.$level = n ? n.$level + 1 : 1, t.$parent = n ? n.id : 0, this.Mg) {
			var a = this.Mg[i || 0];
			if (a || (a = this.Mg[i] = this.order), this.order !== a) {
				var r = a.length;
				!e && this.branch[i || 0].length && (r = 0), a = webix.toArray(a), a.insertAt(t.id, r)
			}
		}
	...
	$dragPos: function(t, e, i) {
			...
			if (l && l == this) {
				...
				if (this.XC = !0, c) {
					if (c != this.Yg) {
						if (c != u) {
							var d, f;
							this.getBranchIndex ? (d = {
								parent: this.getParentId(c)
							}, f = this.getBranchIndex(c)) : (d = {}, f = this.getIndexById(c)), this.callEvent("onBeforeDropOrder", [u, f, e, d]) && (this.move(u, f, this, d), this.Yg = c)
						}
						webix.DragControl.Cd = this.w
					}
				} else if (c = "$webix-last", this.Yg != c) {
					if (!this.callEvent("onBeforeDropOrder", [u, -1, e,
					{
						parent: 0
					}])) return;
					this.Yg = c
		...
		if (i.style.display = "block", h) if (t.y = s.y, t.x = t.x - 18, t.x < s.x) t.x = s.x;
		else {
	...
	$dragIn: function(t, e, i) {
		...
		}, this, [webix.html.pos(i), s], 250), this.$dropAllow(n, i) && this.callEvent("onBeforeDragIn", [n, i]) ? (this.$dragMark(n, i), r) : (n.to = n.target = null, this.bh && (this.bh = window.clearTimeout(this.bh)), null))
	},
	$dropAllow: function() {
		return !0
	},
	...
	$dragOut: function(t, e, i, s) {
		...
		return (a.target || "").toString() == (n || "").toString() ? null : (this.bh && (this.XC = null, this.bh = window.clearTimeout(this.bh)), a.target = a.to = null, this.callEvent("onDragOut", [a, s]), null)
	},
	$drop: function(t, e, i) {
	...
	$dragMark: function(t, e) {
		var i = null;
		return t.target && (i = this._B(t.target)), this.eh && this.eh != i && (t.fragile || this.removeCss(this.eh, "webix_drag_over"), this.eh = null), !this.eh && i ? (this.eh = i, t.fragile || this.addCss(i, "webix_drag_over"), i) : t.to ? !0 : !1
	}
}, webix.Group = {
	$init: function() {
	...
	group: function(t) {
		...
		for (var h = 0; h < s.length; h++) {
			var o = this.id(s[h]);
			this.pull[o] = s[h], this.order.push(o), this.qf && this.qf(s[h])
		}
		...
		for (var o = 0; o < h.length; o++) {
			...
			if (this.hasEvent("onGroupCreated") && this.callEvent("onGroupCreated", [w.id, w.value, this.branch[w.id].lh]), i.footer) {
				...
				for (var b in i.footer) {
					var p = i.footer[b][1] || "any",
						x = this.ih(i.footer[b][0]);
					"function" != typeof p && (p = webix.GroupMethods[p]), g[b] = p.call(this, x, this.branch[h[o].id].lh)
				}
	...
	ph: 0,
	init: function() {
		return null !== this.nh ? this.nh : (webix.destructors.push({
			obj: this
		}), this.nh = document.createElement("textarea"), this.nh.className = "webix_clipbuffer", this.nh.setAttribute("webixignore", 1), document.body.appendChild(this.nh), webix.event(document.body, "keydown", webix.bind(function(t) {
			var e = t.keyCode,
	...
	moveSelection: function(t, e) {
		...
		if (!i.disabled) {
			...
			if (1 == s.length) {
				...
				if (this.OD && (s = this.OD(s, r, t)), this.showItem(s), this.select(s), this.getSubMenu && this.getSubMenu(s) && this.Yq(s, this.getItemNode(s)), !this.config.clipboard) {
	...
	$init: function() {
		this.x.className += " webix_tree", webix.extend(this.data, webix.TreeStore, !0), webix.extend(this.on_click, webix.TreeClick), this.attachEvent("onAfterRender", this.Jf), this.attachEvent("onPartialRender", this.Jf), this.data.provideApi(this, !0), this.x.setAttribute("role", "tree")
	},
	ad: "webix_tm_id",
	on_context: {},
	on_dblclick: {
	...
	vh: function(t) {
		var e, i, s, n, a;
		if (s = [], a = this, t && !a.data.pull[t] && (t = 0), !t || a.data.pull[t].$count) for (i = this.wh(t), i.sort(function(t, e) {
			return a.data.pull[e].$level - a.data.pull[t].$level
		}), e = 0; e < i.length; e++) e && a.data.pull[i[e]].$parent == a.data.pull[i[e - 1]].$parent || (s = s.concat(a.xh(i[e])));
	...
	xh: function(t) {
		var e, i, s, n, a, r, h, o;
		for (n = this.getParentId(t), r = this, a = []; n && "0" != n;) {
	...
	function t(t) {
		return t = t || {}, t.paper = r[(t.paper || "").toLowerCase()] || "A4", t.mode = o[t.mode] ? t.mode : "portrait", t.fit = h[t.fit] ? t.fit : "page", t.scroll = t.scroll || !1, t.size = l[t.paper], t.margin = t.margin || 0 === t.margin ? t.margin : {}, a = isNaN(1 * t.margin) ? a : t.margin, t.margin = {
	...
	function s(t, e) {
		...
		webix.html.insertBefore(i, e.docFooter, document.body), webix.html.addCss(i, "webix_ui_print"), !e.scroll && (t.data && t.data.pull || t.getBody) && webix.html.addCss(i, "webix_print_noscroll"), window.print(), webix.html.remove(i)
	}
	function n(t, e) {
	...
	var a = (webix.env.printPPI, webix.env.printMargin),
		r = {
			a4: "A4",
			a3: "A3",
			letter: "letter"
	...
	webix.print = function(n, a) {
		var r = webix.$$(n);
		r && r.$printView && (r = r.$printView()), r && (r.callEvent("onBeforePrint", [a]), a = t(a), e(a), r.$customPrint && r.$customPrint(a) !== !0 || s(r, a), i(a))
	}
}(), webix.CustomPrint = {
	...
	VF: function(t, e, i) {
		var s, n, a, r = this.XF(t),
			h = t.xCount || this.Ci().Di,
			o = [],
			l = [],
			c = 0;
		i = i || 0, e = e || [];
		...
		return t.forEach(webix.bind(function(s, n) {
			...
			if (s.forEach(function(t) {
				...
				t.forEach(function(t) {
					...
					t.span && (t.span.colspan > 1 && (i.colSpan = t.span.colspan), t.span.rowspan > 1 && (i.rowSpan = t.span.rowspan)), e.appendChild(i)
				}), a.appendChild(e)
	...
	Rh: function(t) {
		...
		return this.Lf(t && e > t), this.s.autoheight && (t || 1 / 0) > e && (t = e), Math.max(this.QF() * t + (this.type.margin || 0), this.s.minHeight || 0)
	},
	QF: function() {
			...
			if (!this.Qh && this.y.innerHTML && webix.animate.isSupported() && this.s.animate && this.Yh != this.Vh.length) {
				if (this.callEvent("onBeforeRender", [this.data])) {
					...
					if (this.Xh) this.Zh.push(this.getScrollState()), webix.Touch && webix.Touch.$active && (o[0].y = 0, o[1].y = -this.getScrollState().y);
						...
						var l = this.Zh.pop();
						webix.Touch && webix.Touch.$active && (o[0].y = -l.y, o[1].y = -this.getScrollState().y)
					}
	...
	_h: function() {
		...
		var n = this.data.$min || 0,
			a = this.data.$max || 1 / 0,
			r = 0;
		for (i in this.units) for (t.push({
			$unit: i
		}), s = this.units[i], e = 0; e < s.length; e++) {
			if (r == n && (t = [{
				$unit: i
			}]), t.push(this.getItem(s[e])), r == a) return webix.toArray(t);
			r++
		}
	...
	ri: function(t, e) {
		if (t) {
			...
			if (this.callEvent("onBeforeEditStop", [i, t, e])) {
				if (!e) {
					var s = i.old;
					if ("string" == typeof i.value && (s += ""), s != i.value || t.config.liveEdit) {
	...
	webix.editors = {
		...
		checkbox: {
			...
			render: function() {
				return webix.html.create("div", {
					"class": "webix_dt_editor"
				}, "<div><input type='checkbox' aria-label='" + i(this.config) + "'></div>")
			}
		},
		select: {
			focus: function() {
		...
		popup: {
			...
			createPopup: function() {
				...
				if (t) {
					var e;
					return "object" != typeof t || t.name ? e = webix.$$(t) : (t.view = t.view || "suggest", e = webix.ui(t)), e.linkInput ? e.linkInput(document.body) : this.linkInput && this.linkInput(document.body), e
				}
		...
		render: function() {
			return webix.html.create("div", {
				"class": "webix_dt_editor"
			}, "<input type='password' aria-label='" + i(this.config) + "'>")
		}
	...
	$init: function() {
		this.t = {}, webix.UE(this.x, "scroll", webix.bind(this.Ai, this)), webix.env.touch && this.attachEvent("onAfterScroll", webix.bind(this.Ai, this)), this.Bi = []
	},
	getItemNode: function(t) {
		return this.t[t]
	},
	...
	Ai: function(t, e) {
		...
		for (var s = i.R; s <= i.Ji;) {
			...
			for (var c = {
				$template: "Loading"
			}, u = 0; u < l.length; u++) l[u] || this.Bi.push(r + u), l[u] = this.jb(l[u] || c);
			o.innerHTML = l.join("");
...
}, webix.DataMove, webix.DragItem, webix.MouseEvents, webix.KeysNavigation, webix.SelectionModel, webix.Scrollable, webix.CustomPrint, webix.ui.proto), webix.DataDriver.htmltable = {
	toObject: function(t) {
		t = webix.toNode(t);
		var e = t.rows;
	...
	$init: function(t) {
		...
		i.className += " webix_vscroll_" + e, i.innerHTML = "<div class='webix_vscroll_body'></div>", webix.UE(i, "scroll", this.Ri, {
			bind: this
		}), this.Si = 0, this.Wi = 0
	},
	Ti: function(t) {
	...
	uB: function(t) {
		!webix.env.touch && window.navigator.pointerEnabled && (webix.html.addCss(t, "webix_scroll_touch_ie", !0), webix.UE(t, "pointerdown", function(t) {
			...
			bind: this
		}), webix.event(window, "pointerup", function(t) {
	...
	dateToStr: function(t, e) {
		return "function" == typeof t ? t : webix.env.strict ?
		function(e) {
			var i = "",
				s = 0;
	...
	strToDate: function(t, e) {
		...
		if (webix.env.strict) return function(t) {
			...
			for (i = 0; i < a.length; i++) {
				...
				else if ("%Y" == r) n[0] = 1 * (s[i] || 0), n[0] < 30 && (n[0] += 2e3);
		...
		case "%F":
			...
			break;
		case "%c":
			r += "var res = date.split('T');", r += "if(res[0]){ var d = res[0].split('-');", r += "set[0]= (d[0]||0)*1; if (set[0]<30) set[0]+=2000;", r += "set[1]= (d[1]||1)-1;", r += "set[2]= d[2]||1;}", r += "if(res[1]){ var t = res[1].split(':');", r += "set[3]= t[0]||0;", r += "set[4]= t[1]||0;", r += "set[5]= parseInt(t[2])||0;}"
		}
	...
	render: function(t, e, i) {
		if ("save" != i) {
			...
			return !this.isVisible(this.s.id) || this.$blockRender ? this.mj() : !t || -1 == e || "paint" != i && "update" != i ? (this.sj && (clearTimeout(this.sj), this.sj = 0), this.callEvent("onBeforeRender", [this.data]) ? (this.mj(), this.tj || this.uj(), this.bc && (this.s.experimental && ("paint" == i || "update" == i) && t ? this.Kv(t) : this.vj(!0, !0)), t && "update" == i || (this.wj = this.xj(), this.yj()), this.x.setAttribute("aria-colcount", Math.max(this.bm.length, this.fj.length)), this.x.setAttribute("aria-rowcount", this.data.count()), this.callEvent("onAfterRender", [this.data]), !0) : void 0) : (this.sj && clearTimeout(this.sj), void(this.sj && this.Lv != t ? (this.Lv = null, this.sj = webix.delay(function() {
				this.render()
	...
	Mj: function(t, e) {
		for (var i = 0, s = 0; s < this.fj.length; s++) {
			...
			for (var a = 0; a < n.length; a++)"object" != typeof n[a] && (n[a] = {
				text: n[a]
			}), n[a] && n[a].height && (e[a] = n[a].height), n[a] && n[a].autoheight && (e[a] = this.RF(n[a], this.fj[s], s));
	...
	Qj: function(t, e, i, s, n) {
		...
		for (var a = "<table role='presentation' style='width:" + i + "px' cellspacing='0' cellpadding='0'>", r = t; e > r; r++) {
			a += "<tr>";
			for (var r = t; e > r; r++) a += "<th  style='width:" + this.fj[r].width + "px'></th>";
			a += "</tr>"
		}
		for (var h = this.fj[0][s].length, o = 0; h > o; o++) {
			...
			for (var r = t; e > r; r++) {
				...
				if (null !== l) {
					...
					var d = n[o],
						f = "";
					l.contentId && (a += " active_id='" + l.contentId + "'"), l.colspan && (a += " colspan='" + l.colspan + "'"), l.rowspan && (a += " rowspan='" + l.rowspan + "'", d = this.Pt(this.Mt, o, l.rowspan)), d != this.s.headerRowHeight && (f = " style='line-height:" + d + "px; height:" + d + "px;'");
					var b = "webix_hcell",
						p = l.css;
	...
	sk: function(t, e) {
		...
		this.oj = s ? webix.ui.scrollSize : 0, webix.env.touch && (i = s = !1), this.Zj && (this.Zj.s.scrollSize = this.nj, this.Zj.s.scrollVisible = i), this.Wj && (this.Wj.s.scrollSize = this.oj, this.Wj.s.scrollVisible = s)
	},
	Hj: function() {
		this.jk = 0, this.bk = 0;
		var t, e;
	...
	nk: function(t, e) {
		...
		for (n += this.fj[i].width, i = 0; i < this.data.order.length && this.data.order[i] != t; i++) r += this.Uj(i);
	...
	lk: function(t) {
		var e = this.jk !== t;
		if (this.jk = t, this.s.prerender) for (var i = this.Vf.childNodes, s = 0; s < i.length; s++) i[s].scrollTop = t;
		else this.vj();
		webix.env.$customScroll && webix.CustomScroll.sk(this.Vf), e && (this.callEvent("onScrollY", []), this.callEvent("onAfterScroll", []))
	},
	kk: function(t) {
		var e = this.bk !== t;
		this.Vf.childNodes[1].scrollLeft = this.bk = t, this.s.header && (this.I.childNodes[1].scrollLeft = t), this.s.footer && (this.cj.childNodes[1].scrollLeft = t), this.s.prerender === !1 && this.vj(this.tk ? !1 : !0), webix.env.$customScroll && webix.CustomScroll.sk(this.Vf), e && (this.callEvent("onScrollX", []), this.callEvent("onAfterScroll", []))
	},
	Xj: function(t) {
	...
	Sj: function(t) {
		...
		var l = a > 0 && e ? -(this.Uj(a - 1) + e) : 0,
			c = a;
		if (e && a--, e += (this.Vj || this.dc) - (this.xD || 0), r) {
			var h = Math.ceil(e / r);
			e -= h * r, c += h
	...
	Kv: function(t) {
		var e = this.getItem(t),
			...
			s = this.Sj();
		if (!(i < s[0] || i >= s[1])) for (var n = this.Xj(), a = 0; a < this.fj.length; a++) {
			var r = this.fj[a];
			if (a < this.Fj && a >= this.s.leftSplit && (a < n[0] || a > n[1]) && (r.qk = -999), r.attached && r.node) {
				var h = r.node.childNodes[i - s[0]],
	...
	yk: function(t, e, i) {
		if (this.GA && (this.GA.style.top = this.ck + "px"), (i || t != this.Bk || e != this.Ck) && (this.Ak(t, e), this.Dk)) {
			...
			for (var s = 0; s < this.wk.length; s++) {
				...
				h.className = "webix_cell " + (r.$sub ? "webix_dtable_sub" + (this.s.subview ? "view" : "row") : "webix_dtable_colrow"), h.setAttribute("column", 0), h.setAttribute("row", a.index);
				var o = r.$height || this.s.rowHeight;
				r.$subopen ? h.style.height = r.$subHeight + "px" : h.style.height = o + "px", h.style.paddingRight = webix.ui.scrollSize + "px", h.style.top = a.top + (r.$subopen ? o - 1 : -1) + "px", this.GA || (this.GA = webix.html.create("DIV"), this.GA.style.position = "relative", this.GA.style.top = this.ck + "px", this.Vf.appendChild(this.GA)), this.GA.appendChild(h), this.attachEvent("onSyncScroll", function(t, e, i) {
	...
	yD: function(t, e, i, s, n) {
		...
		if (h) {
			...
			if (s.single && h.$row && (this.Dk = !0, this.wk.push({
				top: s.total,
				id: h.id,
				index: t
			}), !h.$sub)) return s.total += s.row, "<div" + l + " class='webix_cell'></div>";
			var a = this.Ek(h, e, t),
	...
	yj: function() {
		if (this.fj.length && !isNaN(1 * this.dc)) {
			...
			if (!this.s.autoheight && !this.s.yCount || !this.resize()) {
				this.Wj.sizeTo(this.dc, this.$g, this.jj), this.Wj.define("scrollHeight", t), this.xD = this.s.topSplit * this.s.rowHeight, this.Vj = Math.max(0, this.dc - this.nj - this.$g - this.jj);
	...
	$setSize: function(t, e) {
		var i = this.bc,
			s = this.dc;
		webix.ui.view.prototype.$setSize.apply(this, arguments) && (this.tj && (this.callEvent("onResize", [this.bc, this.dc, i, s]), this.Jj(), this.yj()), this.render())
	},
	Mk: function(t) {
	...
	markSorting: function(t, e) {
		...
		if (i && (i.removeAttribute("aria-sort"), i.removeAttribute("tabindex")), webix.html.remove(this.Pk), e) {
	...
	Qk: function(t) {
		for (var e = this.I.getElementsByTagName("TD"), i = null, s = 0; s < e.length; s++) if (e[s].getAttribute("column") == t && !e[s].getAttribute("active_id") && (i = e[s].firstChild, (e[s].colSpan || 0) < 2)) return i;
		return i
	},
	zf: function(t, e, i) {
	...
	selectFilter: {
		...
		refresh: function(t, e, i) {
			...
			for (var r = document.createElement("select"), h = 0; h < s.length; h++) {
				...
				o.value = s[h].id, o.text = s[h].value, r.add(o)
			}
	...
	init: function(t) {
		t.prepare = function(e, i) {
			var s = -1 != e.indexOf("=") ? 1 : 0,
				n = this.format(e);
			return "" === n ? "" : (-1 != e.indexOf(">") ? t.compare = this.Wk : -1 != e.indexOf("<") ? (t.compare = this.Xk, s *= -1) : (t.compare = this.Yk, s = 0), n - s)
		}
	},
	format: function(t) {
	...
	filterByAll: function() {
		...
		this.data.silent(function() {
			...
			for (var i in this.ij) if (this.isColumnVisible(i)) {
				...
				var r = s[1].compare;
				if (this.callEvent("onBeforeFilter", [i, a, s[1]])) if (s[2].$server || t) t = !0;
				else {
					if ("" === a) continue;
	...
	fC: function(t, e) {
		...
		return s ? ("string" != typeof s && (s = ","), function(t, i, n) {
			if (!t) return !0;
	...
	Zk: function(t, e, i, s) {
		if (this.data.each(function(n) {
			var a = n ? n[t] : "";
			a === webix.undefined || s[a] || (s[a] = !0, i.push({
				id: n[t],
				value: n[e]
			}))
		}, this, !0), i.length) {
			var n = "string" == typeof i[0].value ? "string" : "raw";
			i.sort(this.data.sorting.create({
				as: n,
				by: "value",
				dir: "asc"
			}))
		...
		area: {
			...
			getSelectedId: function(t) {
				var e = this.getSelectArea(),
					i = [];
				if (e) if (!t || e.start.row == e.end.row && e.start.column == e.end.column) i.push(e.end);
				else {
	...
	blockselect_setter: function(t) {
		return t && this.ql && (webix.UE(this.x, webix.env.mouse.move, this.rl, {
			bind: this
		}), webix.UE(this.x, webix.env.mouse.down, this.sl, {
			bind: this
		}), webix.event(document.body, webix.env.mouse.up, this.tl, {
			bind: this
		}), this.ql = this.ul = this.vl = !1, this.attachEvent("onAfterScroll", function() {
			this.ZC()
	...
	tl: function(t) {
		this.yl && (this.jz("select", !0, t), this.yl = webix.html.remove(this.yl)), webix.html.removeCss(document.body, "webix_noselect"), this.ul = this.vl = !1, this.bh && (this.bh = window.clearTimeout(this.bh))
	},
	ZC: function() {
		this.vl && this.jz(!1, !1)
	},
	jz: function(t, e, i) {
		...
		if (this.callEvent("onBeforeBlockSelect", [s, n, e, i])) {
			if ((!this.wB || this.wB(s, n, e, i) !== !1) && s.row && n.row) if ("select" === t) this.il(), this.ol(s, n);
			else {
				var a, r, h, o;
	...
	zl: function(t, e) {
		...
		if (this.s.fixedRowHeight) s = h[c + Math.floor(e / this.s.rowHeight)];
		else for (var l = c; l < h.length; l++) if (o += this.Uj(l), o >= e) {
			s = h[l];
			break
		}
		return s || (s = h[h.length - 1]), {
			row: s,
			column: n
		}
	},
	...
	$init: function(t) {
		...
		}), webix.UE(this.x, webix.env.mouse.down, webix.html.stopEvent), n && (n = "x" == e ? n + " 0 " + n : "0 " + n + " 0 " + n), this.Dl = webix.html.create("DIV", {
	...
	resizeColumn_setter: function(t) {
		return t && this.Kl && (webix.UE(this.x, "mousemove", this.Ll, {
			bind: this
		}), webix.UE(this.x, "mousedown", this.Ml, {
			bind: this
		}), webix.UE(this.x, "mouseup", this.Nl, {
			bind: this
		}), this.Kl = !1), t
	},
	Kl: !0,
	Ml: function(t) {
		this.s.subview && this != webix.$$(t.target || t.srcElement) || this.Ol && (this.Pl = [webix.html.pos(t), this.Ol[2]], webix.html.addCss(document.body, "webix_noselect"), webix.html.denySelect())
	},
	Nl: function() {
	...
	Ql: function(t) {
		if (t = t || event, !this.Rl) {
			...
			if (s) {
				var n, a = this.Pl[0];
				if ("x" == e ? (n = webix.html.offset(i).x + this.Ol[1] - webix.html.offset(this.Vf).x, a = a.x, this.Ol[1] || (s.cind -= i.parentNode.colSpan || 1)) : (n = webix.html.offset(i).y + this.Ol[1] - webix.html.offset(this.Vf).y + this.$g, a = a.y, this.Ol[1] || s.rind--), s.cind >= 0 && s.rind >= 0) {
					this.Rl = [e, s, n];
		...
		if (this.Rl) {
			...
			if ("x" == e) {
				...
				var n = this.fj[i.cind],
					a = n.width;
				delete n.fillspace, delete n.adjust, this.Xs(i.cind, a + s, !0, !0), this.rk()
			} else {
			...
			if (!r || !i.drag) {
				...
				if (this.Ol = !1, r || h) {
					var o = s.offsetWidth,
						...
						u = i.resizeRow;
					"object" == typeof u && u.headerOnly && (e = this.pk(s), e.cind > 0 && (u = !1)), r && u && (u = "object" == typeof u && u.size ? u.size : 3, c.y < u ? (e || (e = this.pk(s)), e.rind && (this.Ol = ["y", 0, s], n = "row-resize")) : l - c.y < u + 1 && (this.Ol = ["y", l, s], n = "row-resize"));
					var d = i.resizeColumn;
					"object" == typeof d && d.headerOnly && r && (d = !1), d && (d = "object" == typeof d && d.size ? d.size : 3, c.x < d ? (this.Ol = ["x", 0, s], n = "col-resize") : o - c.x < d + 1 && (this.Ol = ["x", o, s], n = "col-resize"))
				}
	...
	parse: function(t, e) {
		...
		for (s = 0; s < i.length; s++) if ("undefined" != typeof i[s]) {
			for (var a = i[s].split(e.cols), r = 0; r < a.length; r++) 0 === a[r].indexOf('"') && (a[r] = a[r].substr(1, a[r].length - 2)), a[r] = a[r].replace('""', '"');
			n.push(a)
		}
		return n
	},
	...
	clipboard_setter: function(t) {
		return (t === !0 || 1 === t) && (this.s.clipboard = "block"), webix.clipbuffer.init(), this.attachEvent("onSelectChange", this.Yl), this.attachEvent("onItemClick", function(t, e, i) {
	...
	getState: function() {
		...
		if (i.order = [].concat(this.bm.length ? this.bm : i.ids), this.Nk && (i.sort = {
			...
			for (var h in this.ij) if (!this.am[h]) {
				var o = this.ij[h];
				o[1].value = a[h] = o[2].getValue(o[0]), r = 1
			}
			r && (i.filter = a)
		}
		i.hidden = [];
	...
	setState: function(t) {
		...
		if (t) {
			...
			if (t.filter) {
				...
				this.filterByAll = u, this.filterByAll()
			}
			if (t.select && this.select) {
				var x = t.select;
				this.unselect();
	...
	var i = webix.Touch = {
				...
				if (i.nm) {
					...
					if (r.um) {
						...
						var b = {
							...
							h = webix.$$(i.om);
						h && h.adjustScroll && h.adjustScroll(b), a = Math.max(100, a), s != b.e || n != b.f ? (i.Nf(i.om, b.e, b.f, a + "ms"), i.Dm && i.Dm.Em(b.e, b.f, a + "ms"), i.Fm(b.e, b.f, a + "ms")) : i.Pf()
					} else i.Pf()
				} else if (!this.qm) if (i.zm && !i.ym) i.Gm("onSwipeX");
		...
		em: function(t) {
			if (i.bC && i.km) {
				...
				if (i.Gm("onTouchMove"), i.nm) i.Hm(e);
					...
					if (s) {
						var n = {};
						s.callEvent("onBeforeScroll", [n]), n.update && (i.config.speed = n.speed, i.config.scale = n.scale)
					}
					i.Lm(e)
				}
				return webix.html.preventEvent(t)
		...
		Hm: function() {
			if (i.om) {
				...
				i.jm[0] && (t.e = i.Cm(t.e - e.x + i.lm.x, n, t.e, i.pm.dx, i.pm.px)), i.jm[1] && (t.f = i.Cm(t.f - e.y + i.lm.y, n, t.f, i.pm.dy, i.pm.py)), i.Nf(i.om, t.e, t.f, "0ms"), i.Dm && i.Dm.Em(t.e, t.f, "0ms"), i.Fm(t.e, t.f, "0ms")
			}
		},
		Fm: function(t, e, s) {
		...
		Lm: function(t) {
			-1 != i.nm.indexOf("x") && (i.jm[0] = i.Om("x", i.pm.dx, i.pm.px, "width")), -1 != i.nm.indexOf("y") && (i.jm[1] = i.Om("y", i.pm.dy, i.pm.py, "height")), i.Nm(i.om), window.setTimeout(i.Hm, 1)
		},
		Om: function(t, e, s, n) {
			if (2 > e - s) {
				var a = i.Mf(i.om),
			...
			var o = webix.html.create("DIV", {
				"class": "webix_scroll_" + t
			}, "");
			return o.style[n] = Math.max(s * s / e - 7, 10) + "px", i.pm.left && ("x" === t ? o.style.left = i.pm.left + "px" : o.style.right = -i.pm.left + "px"), i.pm.hidden && (o.style.visibility = "hidden"), i.om.parentNode.appendChild(o), o
		},
		Im: function(t, e, s) {
		...
		Pf: function() {
			var t, e, s;
			s = webix.$$(i.om || this), s && (i.om ? t = i.Mf(i.om) : s.getScrollState && (e = s.getScrollState(), t = {
				e: e.x,
				f: e.y
		...
		dm: function(t) {
			...
			if (!(i.Xf || e.tagName && "textarea" == e.tagName.toLowerCase() && e.offsetHeight < e.scrollHeight)) {
				...
				var n = webix.$$(t);
				!i.cm || i.Sm() || n && n.$touchCapture || (i.bC = null), i.Gm("onTouchStart"), i.Qm(t) && (i.wm = window.setTimeout(i.Tm, i.config.longTouchDelay)), !n || !n.touchable || e.className && 0 === e.className.indexOf("webix_view") || (i.vm = n.getNode(t), webix.html.addCss(i.vm, "webix_touch"))
			}
		},
		Tm: function(t) {
		...
		Rm: function(t, e) {
			...
			}!n || i.om && n.parentNode == i.om.parentNode || (i.Of(), i.Pf(), i.km = s.context(t)), i.em(t)
		},
		Bm: function(t, e) {
			return i.mm = i.lm, i.lm = s.context(t), i.rm.Jm = Math.abs(i.km.x - i.lm.x), i.rm.Ei = Math.abs(i.km.y - i.lm.y), i.mm && (i.lm.time - i.mm.time < i.config.scrollDelay ? (i.rm.sm = i.rm.sm / 1.3 + i.lm.x - i.mm.x, i.rm.tm = i.rm.tm / 1.3 + i.lm.y - i.mm.y) : i.rm.tm = i.rm.sm = 0, i.rm.um = i.rm.um / 1.3 + (i.lm.time - i.mm.time)), i.rm
		},
		Um: function(t) {
			i.pm = {
				dx: t.offsetWidth,
		...
		$init: function() {
			this.attachEvent("onAfterColumnHide", function() {
				...
				if (t) {
					var e = this.bk != -t.e,
						i = this.jk != -t.f;
					webix.Touch.Dm = null, webix.Touch.$m = null, this.jk = 0, this.bk = 0;
					var s = webix.Touch.config.translate;
					return webix.Touch.config.translate = "translate", this.Em(this.Zj ? 0 : t.e, 0, "0ms"), webix.Touch.config.translate = s, this.bk = -t.e, this.jk = -t.f, this.ZF(), this.render(), e && (this.Zj && this.Zj.scrollTo(this.bk), this.callEvent("onScrollX", [])), i && (this.Wj && this.Wj.scrollTo(this.jk), this.callEvent("onScrollY", [])), !1
				}
			})
		},
		Em: function(t, e, i) {
			e += this.jk, t += this.bk, webix.Touch.Nf(this.Vf.childNodes[1].firstChild, t, e, i), this.s.leftSplit && webix.Touch.Nf(this.Vf.childNodes[0].firstChild, 0, e, i), this.s.rightSplit && webix.Touch.Nf(this.Vf.childNodes[2].firstChild, 0, e, i), this.s.header && webix.Touch.Nf(this.I.childNodes[1].firstChild, t, 0, i), this.s.footer && webix.Touch.Nf(this.cj.childNodes[1].firstChild, t, 0, i), this.callEvent("onSyncScroll", [t, e, i])
		},
		Mm: function(t) {
	...
	adjustRowHeight: function(t, e) {
		...
		} else for (var a = new Array(this.data.count()).join("0").split(""), r = this.config.columns, h = 0; h < r.length; h++) this.adjustRowHeight(r[h].id, !0), this.data.each(function(t, e) {
			t.$height > a[e] && (a[e] = t.$height), t.$height = a[e]
		});
		e || this.refresh()
	}
}), webix.extend(webix.ui.datatable, {
	...
	ji: function(t, e, i) {
		...
		var h, o = this.getItem(s),
			l = a.editFormat;
		return this.s.editMath && (h = o["$" + n]), h = h || o[n], webix.isUndefined(h) && (h = ""), e.setValue(l ? l(h) : h, o), e.value = o[n], this.ni(t, e), e.$inline || this.oi(t, r, !0), e.afterRender && e.afterRender(), this.s.liveValidation && (webix.UE(e.node, "keyup", this.Gn(t, this)), this.validateEditor(t)), r
	},
	Gn: function(t, e) {
		return function() {
			e.validateEditor(t)
	...
	pi: function(t) {
		...
		if (h && h.node && h.attached) {
			if (s = this.getIndexById(t.row), this.Rt && (r = this.getSpan(t.row, t.column))) for (i = 0; 3 > i; i++) for (e = this.St[i], n = 0; !o && n < e.childNodes.length; n++) a = e.childNodes[n], a.getAttribute("row") == s && a.getAttribute("column") == this.getColumnIndex(t.column) && (o = a);
	...
	Ln: function() {
		if (this.Eb) {
			...
			this.si(function(t) {
				if (t.getPopup) {
					var i = this.getItemNode(t);
					i ? t.getPopup().show(i) : t.getPopup().show({
						x: -1e4,
						y: -1e4
					})
	...
	hideColumn: function(t, e, i) {
		var s, n = this.s.columns,
			...
			h = 1;
		if (e !== !1) {
			var o = this.getColumnIndex(t);
			if (-1 === o || !this.callEvent("onBeforeColumnHide", [t])) return;
			if (-1 == o) return;
			this.rz();
			var l = n[o].header[0];
			l && (l.$groupSpan = h = l.colspan || 1), o < this.s.leftSplit && (this.s.leftSplit -= h), o >= this.Fj ? this.s.rightSplit -= h : this.Fj -= h;
			for (var c = o + h - 1; c >= o; c--) this.vk(o), s = n.splice(o, 1)[0], r[s.id] = s, s.qk = -1, delete this.Aj[s.id];
			this.callEvent("onAfterColumnHide", [t])
		} else {
			if (s = r[t], !s || !this.callEvent("onBeforeColumnShow", [t])) return;
			for (var u = null, c = 0, d = 0; c < a.length; c++) {
				if (a[c] == t) {
					d = c;
					break
				}
			...
			var o = u ? this.getColumnIndex(u) + 1 : 0,
				l = s.header[0];
			l && (l.colspan = l.$groupSpan || l.colspan, delete l.$groupSpan, h = l.colspan || 1), c < this.Mn[0] && (this.s.leftSplit += h), c >= this.Mn[1] ? this.s.rightSplit += h : this.Fj += h;
			for (var c = d + h - 1; c >= d; c--) {
				var s = r[a[c]];
				webix.PowerArray.insertAt.call(n, s, o), delete s.hidden, delete r[s.id], this.Aj[s.id] = s
			}
			this.callEvent("onAfterColumnShow", [t])
		}
		1 === h && (s.header && this.xt(s, e !== !1 ? 0 : 1, "header"), s.footer && this.xt(s, e !== !1 ? 0 : 1, "footer")), i || this.On()
	},
	xt: function(t, e, i) {
		for (var s = t[i].length - 1; s >= 0; s--) for (var n, a = this.bm, r = !1, h = 0, o = 0; o < a.length; o++) {
			var t = this.getColumnConfig(a[o]),
				l = t[i][s];
			this.isColumnVisible(a[o]) ? (r && h > 0 && n && n.colspan > 0 ? (l = t.el[s] = n, n = l) : l && l.$colspan && 0 >= h && (h = l.colspan = l.$colspan, n = l), r = null) : (l && l.$colspan && 0 >= h && (h = l.colspan = l.$colspan, r = n = l), n && h > 0 && n.colspan--), h--
		}
	},
	...
	showColumnBatch: function(t, e) {
		...
		e = e !== !1, this.eachColumn(function(s, n) {
			if (n.batch) {
				var a = this.am[n.id];
				e || (a = !a), n.batch == t && a ? this.hideColumn(n.id, !e, !0) : i || n.batch == t || a || this.hideColumn(n.id, e, !0)
			}
		}, !0), this.On()
	}
}), webix.extend(webix.ui.datatable, {
		...
		if (!this.s.disabled) {
			...
			if (s >= 0) {
				...
				if ("top" == t || "bottom" == t) a && ("top" == t ? a = this.data.getFirstId() : "bottom" == t && (a = this.data.getLastId())), r && (s = 0, "bottom" == t && (s = this.config.columns.length - 1), r = this.columnId(s));
					...
					if (a) {
						var s = this.getIndexById(a),
							h = "pgup" == t || "pgdown" == t ? Math.round(this.Vj / this.s.rowHeight) : 1;
						"up" == t || "pgup" == t ? s -= h : ("down" == t || "pgdown" == t) && (s += h), 0 > s && (s = 0), s >= this.data.order.length && (s = this.data.order.length - 1), a = this.getIdByIndex(s), !a && this.s.pager && this.showItemByIndex(s)
					}
				} else {
					if ("right" != t && "left" != t) return;
					if (r && "row" != this.config.select) {
						var s = this.getColumnIndex(r);
						"right" == t ? s++ : "left" == t && s--, 0 > s && (s = 0), s >= this.config.columns.length && (s = this.config.columns.length - 1), r = this.columnId(s)
					} else {
	...
	getHeaderNode: function(t, e) {
		if (this.isColumnVisible(t)) {
			...
			e = e || 0;
			for (var n = this.I.childNodes[s].getElementsByTagName("TR")[e + 1].childNodes, a = 0; a < n.length; a++) if (n[a].getAttribute("column") == i) return n[a].firstChild
		}
		return null
	},
	...
	dragColumn_setter: function(t) {
		...
		"order" == t ? e = {
			...
			$dragPos: webix.bind(function(t, e, i) {
				...
				if (r && r.column == h && (this.Sn = !1), i.style.display = "block", t.x = t.x - this.Qn.x, t.y = n.y, t.x < n.x) t.x = n.x;
				else {
			...
			$dragIn: webix.bind(function(t, i, s) {
				...
				for (var a = s.target || s.srcElement; - 1 == (a.className || "").indexOf("webix_hcell");) if (a = a.parentNode, !a) return;
	...
	e.draw = function(e, i, s, n) {
		...
		return t = webix.color.toRgb(t), t.push(e), "rgba(" + t.join(",") + ")"
	}, e.DC = function(t) {
	...
	t.prototype.draw = function(t, e, i) {
		var s, n, a, r, h, o = this.config,
			...
			c = webix.EC;
		return r = this.getPoints(t, e, i), a = c.definePath(l.FC(r), !0), o.color && (h = this.GC(c, o.color)), n = c.group(c.getPath(a, "webix_sparklines_area" + (h ? " " + h.area : ""))), r.splice(r.length - 3, 3), a = c.definePath(l.FC(r)), n += c.group(c.getPath(a, "webix_sparklines_line" + (h ? " " + h.line : ""))), n += l.HC(c, r, o.radius, "webix_sparklines_item" + (h ? " " + h.item : "")), s = Math.min(t.length ? (e - 2 * (o.paddingX || 0)) / t.length : 0, o.eventRadius), n += l.IC(c, r, s), c.draw(n, e, i, "webix_sparklines_area_chart" + (o.css ? " " + o.css : ""))
	...
	t.prototype.draw = function(t, e, i) {
		var s, n, a, r, h, o = this.config,
			...
			d = webix.EC;
		for (s = 0; s < u.length; s++) n = "function" == typeof o.itemCss ? o.itemCss.call(this, t[s]) : o.itemCss || "", o.negativeColor && t[s] < o.origin ? n += " " + this.GC(d, o.negativeColor) : o.color && (n += " " + this.GC(d, o.color)), a = u[s], c.push(d.getRect(a.x, a.y, a.width, a.height, "webix_sparklines_bar " + n));
		...
		var f = this.KC(t, e, i),
			b = [];
		for (s = 0; s < f.length; s++) a = f[s], b.push(d.getRect(a.x, a.y, a.width, a.height, "webix_sparklines_event_area ", {
			webix_area: s
		}));
		...
		var i = {},
			s = t.styleMap;
		return e && (i[s.color] = e), webix.html.createCss(i)
	...
	t.prototype.draw = function(t, e, i) {
		...
		e = (e || 100) - 2 * h;
		var l = s.minHeight || 0;
		if (i = (i || 100) - 2 * o, t.length) if (1 == t.length) r.push({
			x: e / 2 + h,
			y: i / 2 + h
		});
		else {
	...
	t.prototype.rC = 0, t.prototype.qC = ["#f55b50", "#ff6d3f", "#ffa521", "#ffc927", "#ffee54", "#d3e153", "#9acb61", "#63b967", "#21a497", "#21c5da", "#3ea4f5", "#5868bf", "#7b53c0", "#a943ba", "#ec3b77", "#9eb0b8"], t.prototype.xv = function(t, e) {
		...
		for ("function" != typeof o && (o = function() {
			return o
		}), r = "", a = 0; a < l.length; a++) s = {}, s[c.styleMap.color] = o.call(this, a, t, this.LC), r += c.getSector({
			x: f,
			y: b
		}, d, l[a][0], l[a][1], "webix_sparklines_sector", s);
		for (n = c.group(r), r = "", a = 0; a < l.length; a++) r += c.getSector({
			x: f,
			y: b
		}, d, l[a][0], l[a][1], "webix_sparklines_event_area", {
			webix_area: a
		});
		...
		var e, i, s = [],
			n = 0,
			a = this.Vo(t);
		for (e = 0; e < t.length; e++) i = 1 * t[e], s[e] = 2 * Math.PI * (a ? (i + n) / a : 1 / t.length), n += i;
		return s
	...
	t.prototype.draw = function(t, e, i) {
		...
		for (n = c.getPoints.call(this, t, e, i), s = 0; s < n.length; s++) o.push(n[s].x), l.push(n[s].y);
		...
		var e, i, s = [],
			...
			o = [],
			l = t.length - 1;
		for (s[0] = 0, n[0] = 2, a[0] = 1, r[0] = t[0] + 2 * t[1], e = 1; l - 1 > e; e++) s[e] = 1, n[e] = 4, a[e] = 1, r[e] = 4 * t[e] + 2 * t[e + 1];
	...
	UF: function(t, e) {
		return t.scroll && !e ? !0 : (t.header = webix.isUndefined(t.header) ? this.config.header ? !0 : !1 : t.header, t.footer = webix.isUndefined(t.footer) ? this.config.footer ? !0 : !1 : t.footer, void(t.xCorrection = t.xCorrection || 0))
	},
	sq: function(t, e) {
			...
			h.forEach(webix.bind(function(t, a) {
				for (var h = e[a + n], c = 0; c < h[i].length; c++) {
					...
					if (u || s[r] && s[r][c]) {
						if (u = webix.copy(u || {
							text: ""
						}), s[r] && s[r][c] && 0 === a && (u.colspan = s[r][c], s[r][c] = 0), u.colspan) {
							var d = Math.min(u.colspan, l - a);
							s[r + 1] = s[r + 1] || {}, s[r + 1][c] = u.colspan - d, u.colspan = d
						}
						u.rowspan && 1 === l && (u.height = (u.height || this.config.headerRowHeight) * u.rowspan, u.rowspan = null);
						var f = {
			...
			for (var c = 0, u = this.getItem(e), d = this.getIndexById(e), f = [], b = !1, p = i; p < s.length; p++) {
				...
				if (s[p]) {
					...
					if ("selection" !== t.data || "selection" == t.data && -1 !== this.sq(n, function(t) {
						...
						var v = this.getSpan(e, x);
						if (v && this.getColumnIndex(x) === i) {
							var g = this.getColumnIndex(v[1]);
							i > g && (v[2] = v[2] - (i - g), v[4] = v[4] ? v[4] : u[v[1]] ? this.getText(e, v[1]) : null, v[1] = x)
						}
						if (!v || v && v[0] == e && v[1] == x) {
							var m = v && v[4] ? v[4] : u[x] ? this.getText(e, x) : "",
								y = this.getCss(e, x) + " " + (s[p].css || "") + (v ? " webix_dtable_span " + (v[5] || "") : ""),
								$ = {
									height: v && v[3] > 1 ? "auto" : (u.$height || this.config.rowHeight) + "px",
									width: v && v[2] > 1 ? "auto" : s[p].width + "px"
								};
							f.push({
								txt: m,
								className: y,
								...
								span: v ? {
									colspan: v[2],
									spanStart: this.getColumnIndex(v[1]),
									rowspan: v[3]
								} : null
							}), m && (r = Math.max(w + 1, r), h = Math.max(d + 1, h)), b = b || !! m
						} else v && (f.push({
							$inspan: !0
						}), r = Math.max(w + 1, r), h = Math.max(d + 1, h))
					}
				}
			}(!t.skiprows || b) && o.push(f)
		}, this)), h && r && (t.trim && (o.length = h, o = o.map(function(t) {
			for (var e = t.length - 1; e >= 0; e--) if (t[e].span && t[e].span.colspan) {
				t[e].span.colspan = Math.min(t[e].span.colspan, t.length - e);
				break
			}
			return t.length = r, t
		})), e.push(o)), l ? this.VF(t, e, l) : (t.header && (e = this.$F(e, s, "header")), t.footer && (e = this.$F(e, s, "footer"))), e
	},
	WF: function(t, e) {
		var i = webix.html.create("div");
		return t.forEach(webix.bind(function(s, n) {
	...
	freezeRow: function(t, e) {
		...
		var s, n = this.s.topSplit,
			a = this.data.order,
			r = this.data.jf;
		t ? (s = this.getIndexById(t), t = t.toString(), i(s, t, e, a), r && i(r.find(t), t, e, r, !0)) : e || (n = 0), this.define("topSplit", n), this.refresh()
	}
	...
	addSpan: function(t, e, i, s, n, a) {
		if ("object" != typeof t) s = s || 1, i = i || 1, this.Rt[t] || (this.Rt[t] = {}), this.Rt[t][e] = [i, s, n, a];
		else for (var r = 0; r < t.length; r++) this.addSpan.apply(this, t[r])
	},
	...
	mA: function(t, e) {
		...
		i.style.cssText = "width:" + this.bc + "px; height:auto; visibility:hidden; position:absolute; top:0px; left:0px; overflow:hidden;", this.$view.appendChild(i), this.data.each(function(s) {
	...
	_t: function(t) {
		var e = {
			...
			on: {
				onMenuItemClick: webix.bind(function(t, e) {
					var i = webix.$$(this.s.headermenu),
						s = i.getItem(t).hidden;
					return i.getItem(t).hidden = !s, i.refresh(t), i.$blockRender = !0, s ? this.showColumn(t) : this.hideColumn(t), i.$blockRender = !1, !1
				}, this)
			},
			data: []
		};
	...
	refresh: function(t, e, i) {
		if (!t.$destructed) {
			...
			var h = s.getPopup().getList(),
				o = webix.$$(r);
			if (o && o.data && o.data.getRange && (a = o.data.getRange()), e.firstChild.appendChild(s.$view.parentNode), h.parse && (h.clearAll(), h.parse(a), !this.$noEmptyOption && i.emptyOption !== !1 || i.emptyOption)) {
				var l = {
					id: "",
					value: i.emptyOption || "",
					$empty: !0
	...
	refresh: function(t, e, i) {
		e.onclick = function(e) {
			...
			i.closed ? (i.closed = !1, s.className = "webix_tree_open") : (i.closed = !0, s.className = "webix_tree_close"), webix.delay(function() {
	...
	renderTextAt: function(t, e, i, s, n, a, r) {
		...
		return h && (t && ("middle" == t ? h.style.top = parseInt(s - h.offsetHeight / 2, 10) + "px" : h.style.top = s - h.offsetHeight + "px"), e && ("left" == e ? h.style.left = i - h.offsetWidth + "px" : h.style.left = parseInt(i - h.offsetWidth / 2, 10) + "px")), h
	},
	...
	rgbToHsv: function(t, e, i) {
		var s, n, a, r, h, o, l, c;
		return s = t / 255, n = e / 255, a = i / 255, r = Math.min(s, n, a), h = Math.max(s, n, a), l = 0, o = 0 === h ? 0 : 1 - r / h, c = h, h == r ? l = 0 : h == s && n >= a ? l = 60 * (n - a) / (h - r) + 0 : h == s && a > n ? l = 60 * (n - a) / (h - r) + 360 : h == n ? l = 60 * (a - s) / (h - r) + 120 : h == a && (l = 60 * (s - n) / (h - r) + 240), [l, o, c]
	}
	...
	hide: function(t, e, i) {
		if (t.querySelectorAll) for (var s = t.querySelectorAll('area[userdata="' + e + '"]'), n = 0; n < s.length; n++) s[n].style.display = i ? "none" : ""
	},
	render: function(t) {
		var e = webix.html.create("DIV");
	...
	type_setter: function(t) {
		return "undefined" == typeof this.s.offset && (this.s.offset = !(-1 != t.toLowerCase().indexOf("area"))), "radar" != t || this.s.yAxis || this.define("yAxis", {}), "scatter" == t && (this.s.yAxis || this.define("yAxis", {}), this.s.xAxis || this.define("xAxis", {})), t
	},
	destructor: function() {
	...
	render: function(t, e, i) {
		...
		if (this.isVisible(this.s.id) && (e = this.oo(), this.callEvent("onBeforeRender", [e, i]))) {
			if (this.canvases && "object" == typeof this.canvases) for (n in this.canvases) this.canvases[n].clearCanvas();
			else this.canvases = {};
			if (this.s.legend && (this.canvases.legend || (this.canvases.legend = this.lo("legend")), this.mo(this.data.getRange(), this.bc, this.dc)), this.eo = a = new webix.HtmlMap(this.ad), r = this.s, s = this.no(this.bc, this.dc), this.go) for (n = 0; n < this.go.length; n++) this.s = this.go[n], this.canvases[n] || (this.canvases[n] = this.lo(this.s.ariaLabel + " " + n, "z-index:" + (2 + n), null, n)), this["$render_" + this.s.type](this.canvases[n].getCanvas(), e, s.start, s.end, n, a);
	...
	oo: function() {
		...
		if (s = this.data.getRange(), t = -1 != this.s.type.toLowerCase().indexOf("barh") ? "yAxis" : "xAxis", e = this.s[t], e && e.units && "object" == typeof e.units) {
			if (i = e.units, h = [], "undefined" != typeof i.start && "undefined" != typeof i.end && "undefined" != typeof i.next) for (r = i.start; r <= i.end;) h.push(r), r = i.next.call(this, r);
			else "[object Array]" === Object.prototype.toString.call(i) && (h = i);
			if (a = [], h.length) {
				for (o = e.value, l = {}, n = 0; n < s.length; n++) l[o(s[n])] = n;
				for (n = 0; n < h.length; n++)"undefined" != typeof l[h[n]] ? (s[l[h[n]]].$unit = h[n], a.push(s[l[h[n]]])) : a.push({
					$unit: h[n]
				})
			}
			return a
	...
	uo: function(t, e, i, s, n, a) {
		var r, h = 0;
		return this.s.yAxis && (this.canvases.y || (this.canvases.y = this.lo("axis_y")), h = this.vo(this.canvases.y.getCanvas(), t, e, i, s, n)), this.s.xAxis && (this.canvases.x || (this.canvases.x = this.lo("axis_x")), r = this.canvases.x.getCanvas(), this.callEvent("onBeforeXAxis", [r, t, e, i, a, h]) && this.wo(r, t, e, i, a, h)), h
	},
	...
	vo: function(t, e, i, s, n, a) {
		...
		if (this.s.yAxis) {
			var o = i.x - .5,
				...
				c = i.y,
				u = s.y + .5;
			if (this.s.yAxis.step && (r = parseFloat(this.s.yAxis.step)), "undefined" == typeof this.to.step || "undefined" == typeof this.to.start || "undefined" == typeof this.to.end ? (h = this.Ao(n, a), n = h.start, a = h.end, r = h.step, this.s.yAxis.end = a, this.s.yAxis.start = n) : "logarithmic" == this.config.scale && (this.Bo = !0), this.Co(i, s), 0 !== r) {
				if (a == n) return l;
				for (var d = (l - c) * r / (a - n), f = 0, b = n; a >= b; b += r) {
					...
					if (b == n && "auto" == this.s.origin || !this.s.yAxis.lines.call(this, b) || this.zo(t, o, x, s.x, x, this.s.yAxis.lineColor.call(this, b), 1), b == this.s.origin && (u = x), 1 > r && !this.Bo) {
						var w = Math.min(Math.floor(this.Do(r)), 0 >= n ? 0 : Math.floor(this.Do(n))),
							v = Math.pow(10, -w);
						p = Math.round(p * v) / v, b = p
					}
	...
	Ao: function(t, e) {
		...
		if (l = l > 5 ? 10 : 5, n = parseInt(l, 10) * o, n > Math.abs(t)) a = 0 > t ? -n : 0;
			...
			var c = Math.abs(t),
				...
				d = c / Math.pow(10, u);
			for (a = Math.ceil(10 * d) / 10 * Math.pow(10, u) - n, c > 1 && n > .1 && (a = Math.ceil(a)); 0 > t ? t >= a : a >= t;) a -= n;
			0 > t && (a = -a - 2 * n)
		}
	...
	Go: function(t, e) {
		...
		else if (i = webix.GroupMethods.max(this.go[0][e], n), s = a && "undefined" != typeof a.start ? parseFloat(a.start) : webix.GroupMethods.min(this.go[0][e], n), this.go.length > 1) for (var r = 1; r < this.go.length; r++) {
			var h = webix.GroupMethods.max(this.go[r][e], n),
	...
	mo: function(t, e) {
		...
		}, ""), s.padding && (n.style.padding = s.padding + "px"), this.ko = n, this.w.appendChild(n), r = [], s.values) for (i = 0; i < s.values.length; i++) r.push(this.Jo(n, s.values[i].text, "undefined" != typeof s.values[i].id ? typeof s.values[i].id : i, s.values[i].$hidden));
		else for (i = 0; i < t.length; i++) r.push(this.Jo(n, s.template(t[i])));
	...
	Ko: function(t, e, i, s, n, a, r) {
		...
		if (s && (t.strokeStyle = t.fillStyle = s), "round" != c && o.radius) if ("item" == c) {
			...
			p && (t.beginPath(), a ? (t.lineWidth = b.borderWidth, t.strokeStyle = s, t.fillStyle = s) : (t.lineWidth = b.borderWidth, t.fillStyle = b.color.call(this, {}), t.strokeStyle = b.borderColor.call(this, {}), t.globalAlpha = b.alpha.call(this, {})), t.beginPath(), e += o.width / 2 + 5, i += n / 2, this.Lo(t, e, i, p + 1, b.type), t.fill(), t.stroke()), t.globalAlpha = 1
	...
	no: function(t, e) {
		var i, s, n, a;
		if (i = this.s.padding.left, s = this.s.padding.top, n = t - this.s.padding.right, a = e - this.s.padding.bottom, this.s.legend) {
			var r = this.s.legend,
	...
	No: function(t) {
		...
		if (this.s.yAxis && "undefined" != typeof this.s.yAxis.end && "undefined" != typeof this.s.yAxis.start && this.s.yAxis.step) s = parseFloat(this.s.yAxis.end), n = parseFloat(this.s.yAxis.start);
			...
			for (e = 0; e < t.length; e++) for (t[e].$sum = 0, t[e].$min = 1 / 0, i = 0; i < this.go.length; i++) a = parseFloat(this.go[i].value(t[e]) || 0), isNaN(a) || (-1 != this.go[i].type.toLowerCase().indexOf("stacked") && (t[e].$sum += a), a < t[e].$min && (t[e].$min = a));
	...
	$o: function(t, e, i, s, n, a, r) {
		if (t.lineWidth = 1, 0 >= s && n >= 0 || s >= 0 && n <= Math.PI || Math.abs(s - Math.PI) > .003 && s <= Math.PI && n >= Math.PI) {
			...
			t.beginPath(), t.arc(e, i, a, s, n, !1), t.lineTo(e + a * Math.cos(n), i + a * Math.sin(n) + h), t.arc(e, i + h, a, n, s, !0), t.lineTo(e + a * Math.cos(s), i + a * Math.sin(s)), t.fill(), r && t.stroke()
		}
	},
	...
	Xo: function(t, e, i, s) {
		t.globalAlpha = .5;
		for (var n = ["#c4c4c4", "#c6c6c6", "#cacaca", "#dcdcdc", "#dddddd", "#e0e0e0", "#eeeeee", "#f5f5f5", "#f8f8f8"], a = n.length - 1; a > -1; a--) t.beginPath(), t.fillStyle = n[a], t.arc(e + 1, i + 1, s + a, 0, 2 * Math.PI, !0), t.fill();
		t.globalAlpha = 1
	},
	ap: function(t) {
	...
	Yo: function(t, e, i, s, n, a) {
		t.beginPath();
		var r;
		"function" != typeof this.s.gradient ? (r = t.createRadialGradient(n, a, s / 4, e, i, s), r = this.ap(r)) : r = this.s.gradient(r), t.fillStyle = r, t.arc(e, i, s, 0, 2 * Math.PI, !0), t.fill(), t.globalAlpha = .7
	},
	Zo: function(t, e, i, s, n, a, r, h) {
		...
		if (o) {
			...
			var x = (e + Math.floor((i + p) * Math.sin(u))) * a + f,
				...
				v = n < Math.PI / 2 + .01,
				g = s < Math.PI / 2;
			g && v ? w = Math.max(w, t + 3) : g || v ? !h && (u >= Math.PI / 2 && u < Math.PI || u <= 3 * Math.PI / 2 && u >= Math.PI) && (w += l / 3) : w = Math.min(w, t - l), o.style.top = x + "px", o.style.left = w + "px", o.style.width = l + "px", o.style.textAlign = b, o.style.whiteSpace = "nowrap"
		}
	},
	...
	$render_bar: function(t, e, i, s, n, a) {
		...
		var $ = (h - r * m) / 2,
			...
			C = !1,
			S = this.s.gradient;
		for (S && "function" != typeof S ? (C = S, S = !1) : S && (S = t.createLinearGradient(0, s.y, 0, i.y), this.s.gradient(S)), w || this.zo(t, i.x, s.y + .5, s.x, s.y + .5, "#000000", 1), o = 0; o < e.length; o++) {
			var I = parseFloat(this.s.value(e[o]) || 0);
			if (this.Bo && (I = this.Do(I)), I && !isNaN(I)) {
				I > c && (I = c), I -= u, I *= f;
				var E = i.x + $ + o * h + (r + 1) * y,
					k = s.y;
				if (0 > I || this.s.yAxis && 0 === I && !("auto" != this.s.origin && this.s.origin > u)) this.canvases[n].renderTextAt(!0, !0, E + Math.floor(r / 2), k, this.s.label(e[o]));
				else {
					v || "auto" != this.s.origin && w || (I += p / x);
					var A = S || this.s.color.call(this, e[o]);
					t.globalAlpha = this.s.alpha.call(this, e[o]);
					var M = this.bp(t, i, E, k, r, u, _, x, I, A, S, C);
					C && this.cp(t, E, k, r, u, _, x, I, A, C), this.s.border && this.dp(t, E, k, r, u, _, x, I, A), t.globalAlpha = 1, M[0] != E ? this.canvases[n].renderTextAt(!1, !0, E + Math.floor(r / 2), M[1], this.s.label(e[o])) : this.canvases[n].renderTextAt(!0, !0, E + Math.floor(r / 2), M[3], this.s.label(e[o])), a.addRect(e[o].id, [E - i.x, M[3] - i.y, M[2] - i.x, M[1] - i.y], n)
				}
			}
		}
	},
	...
	dp: function(t, e, i, s, n, a, r, h, o) {
		var l;
		t.save(), l = this.ep(t, e, i, h, r, s, n), this.gp(t, o), this.fp(t, l.x0, l.y0, s, a, r, l.value, t.lineWidth / 2, 1), t.stroke(), t.restore()
	},
	cp: function(t, e, i, s, n, a, r, h, o, l) {
		...
		var c = this.ep(t, e, i, h, r, s, n),
			...
			d = this.s.border ? 1 : 0;
		t.fillStyle = u.gradient, this.fp(t, c.x0 + u.offset, c.y0, s - 2 * u.offset, a, r, c.value, u.offset + d), t.fill(), t.restore()
	},
	...
	$render_line: function(t, e, i, s, n, a) {
		...
		if (l = this.hp(t, e, i, s, n), r = this.s, e.length) {
			for (c = r.offset ? i.x + .5 * l.cellWidth : i.x, o = [], h = 0; h < e.length; h++) if (w = this.ip(e[h], i, s, l), w || "0" == w) {
				if (d = h ? l.cellWidth * h - .5 + c : c, b = "object" == typeof w ? w.y0 : w, h && this.s.fixOverflow) {
					...
					u = l.cellWidth * (h - 1) - .5 + c, f = "object" == typeof x ? x.y0 : x, x.out && (p = "min" == x.out ? s.y : i.y, o.push({
						x: this.jp(u, d, f, b, p),
						y: p
					})), w.out && (p = "min" == w.out ? s.y : i.y, o.push({
						x: this.jp(u, d, f, b, p),
						y: p
					}))
				}
	...
	Lo: function(t, e, i, s, n) {
		var a = [];
		if (!n || "square" != n && "s" != n) if (!n || "diamond" != n && "d" != n) a = !n || "triangle" != n && "t" != n ? [
			[e, i, s, 0, 2 * Math.PI, !0]
		] : [
			[e, i - s],
	...
	hp: function(t, e, i, s, n) {
		...
		return a = l[0], r.valueFactor = l[1], r.unit = a ? r.totalHeight / a : 10, r.startValue = 0, h || (r.startValue = 10, r.unit != r.totalHeight && (r.unit = a ? (r.totalHeight - r.startValue) / a : 10)), r
	}
}), webix.extend(webix.ui.chart, {
	$render_barH: function(t, e, i, s, n, a) {
		var r, h, o, l, c, u, d, f, b, p, x, w, v, g, m, y, $, _, C, S, I;
		for (o = (s.y - i.y) / e.length, d = this.Go("h"), f = d.max, b = d.min, y = s.x - i.x, I = !! this.s.xAxis, n || this.mp(t, e, i, s, b, f, o), I && (f = parseFloat(this.s.xAxis.end), b = parseFloat(this.s.xAxis.start)), g = this.Ho(b, f), w = g[0], x = g[1], _ = w ? y / w : 10, I || (m = 10, _ = w ? (y - m) / w : 10), h = parseInt(this.s.barWidth, 10), h * this.go.length + 4 > o && (h = o / this.go.length - 4), r = Math.floor((o - h * this.go.length) / 2), v = "undefined" != typeof this.s.radius ? parseInt(this.s.radius, 10) : Math.round(h / 5), p = !1, c = this.s.gradient, c && "function" != typeof c ? (p = c, c = !1) : c && (c = t.createLinearGradient(i.x, i.y, s.x, i.y), this.s.gradient(c)), I || this.zo(t, i.x - .5, i.y, i.x - .5, s.y, "#000000", 1), u = 0; u < e.length; u++) if ($ = parseFloat(this.s.value(e[u] || 0)), this.Bo && ($ = this.Do($)), $ && !isNaN($)) if ($ > f && ($ = f), $ -= b, $ *= x, C = i.x, S = i.y + r + u * o + (h + 1) * n, 0 > $ && "auto" == this.s.origin || this.s.xAxis && 0 === $ && !("auto" != this.s.origin && this.s.origin > b)) this.canvases[n].renderTextAt("middle", "right", C + 10, S + h / 2 + r, this.s.label(e[u]));
		else {
			0 > $ && "auto" != this.s.origin && this.s.origin > b && ($ = 0), I || ($ += m / _), l = c || this.s.color.call(this, e[u]), this.s.border && this.np(t, C, S, h, b, v, _, $, l), t.globalAlpha = this.s.alpha.call(this, e[u]);
			var E = this.op(t, s, C, S, h, b, v, _, $, l, c, p);
			p && this.pp(t, C, S, h, b, v, _, $, l, p), t.globalAlpha = 1, E[3] == S ? (this.canvases[n].renderTextAt("middle", "left", E[0] - 5, E[3] + Math.floor(h / 2), this.s.label(e[u])), a.addRect(e[u].id, [E[0] - i.x, E[3] - i.y, E[2] - i.x, E[3] + h - i.y], n)) : (this.canvases[n].renderTextAt("middle", !1, E[2] + 5, E[1] + Math.floor(h / 2), this.s.label(e[u])), a.addRect(e[u].id, [E[0] - i.x, S - i.y, E[2] - i.x, E[3] - i.y], n))
		}
	},
	...
	rp: function(t, e, i, s, n, a) {
		...
		if (o) {
			var l = s.y + .5,
				...
				u = s.x - .5,
				d = i.x;
			if (this.zo(t, c, l, u, l, o.color, 1), o.step && (r = parseFloat(o.step)), ("undefined" == typeof this.so.step || "undefined" == typeof this.so.start || "undefined" == typeof this.so.end) && (h = this.Ao(n, a), n = h.start, a = h.end, r = h.step, this.s.xAxis.end = a, this.s.xAxis.start = n, this.s.xAxis.step = r), 0 !== r) {
				for (var f = (u - c) * r / (a - n), b = 0, p = n; a >= p; p += r) {
					...
					if (p == n && "auto" == this.s.origin || !o.lines.call(this, p) || this.zo(t, w, l, w, i.y, this.s.xAxis.lineColor.call(this, p), 1), p == this.s.origin && (d = w + 1), 1 > r && !this.Bo) {
						var v = Math.min(Math.floor(this.Do(r)), 0 >= n ? 0 : Math.floor(this.Do(n))),
							g = Math.pow(10, -v);
						x = Math.round(x * g) / g, p = x
					}
					this.canvases.x.renderTextAt(!1, !0, w, l + 2, o.template(x.toString()), "webix_axis_item_x"), b++
				}
	...
	$render_stackedBar: function(t, e, i, s, n, a) {
		...
		for (var S = 0; S < e.length; S++) {
			...
			this.Bo && (I = this.Do(I)), l = i.x + _ + S * g;
			var E = v && 0 > I;
			if (n ? c = E ? e[S].$startYN : e[S].$startY : (c = o - 1, e[S].$startY = c, v && (E && (c = o + 1), e[S].$startYN = o + 1)), I && !isNaN(I) && (n || v || (I -= h), I *= u, !(c < i.y + 1))) {
				...
				var M = c - y * I + (A ? E ? -1 : 1 : 0),
					D = this.up(t, l - (f.border ? .5 : 0), c, $ + (f.border ? .5 : 0), M, 0, i.y);
				if (t.fill(), t.stroke(), C) {
					t.save();
					var N = this.Oo(t, l, c, l + $, D[1], C, k, "y");
					t.fillStyle = N.gradient, t.beginPath(), D = this.up(t, l + N.offset, c, $ - 2 * N.offset, M, f.border ? 1 : 0, i.y), t.fill(), t.restore()
				}
				f.border && (t.save(), "string" == typeof f.border ? t.strokeStyle = f.border : this.gp(t, k), t.beginPath(), this.up(t, l - .5, parseInt(c, 10) + .5, $ + 1, parseInt(M, 10) + .5, 0, i.y, A), t.stroke(), t.restore()), t.globalAlpha = 1, this.canvases[n].renderTextAt(!1, !0, l + Math.floor($ / 2), D[1] + (c - D[1]) / 2 - 7, this.s.label(e[S])), a.addRect(e[S].id, [l - i.x, D[1] - i.y, D[0] - i.x, e[S][E ? "$startYN" : "$startY"] - i.y], n), e[S][E ? "$startYN" : "$startY"] = D[1]
			}
		}
	},
	...
	$render_stackedBarH: function(t, e, i, s, n, a) {
		...
		for (var C = 0; C < e.length; C++) {
			...
			if (_ ? I = e[C].$startX : e[C].$startX = I, S && !isNaN(S)) {
				...
				var A = this.qp(t, I, E, w, g, p, S, 0);
				if (y && !m && t.lineTo(i.x + c, E + (this.s.border ? 1 : 0)), t.fill(), m) {
					var M = this.Oo(t, I, E + w, I, E, m, k, "x");
					t.fillStyle = M.gradient, t.beginPath(), A = this.qp(t, I, E, w, g, p, S, 0), t.fill()
				}
				this.s.border && this.np(t, I, E, w, h, g, p, S, k), t.globalAlpha = 1, this.canvases[n].renderTextAt("middle", !0, e[C].$startX + (A[0] - e[C].$startX) / 2 - 1, E + (A[1] - E) / 2, this.s.label(e[C])), a.addRect(e[C].id, [e[C].$startX - i.x, E - i.y, A[0] - i.x, A[1] - i.y], n), e[C].$startX = A[0]
			}
		}
	}
}), webix.extend(webix.ui.chart, {
	...
	vp: function(t) {
		...
		for (h = [], o = [], h[0] = 0, h[1] = 2 * (l[0] + l[1]), o[0] = 0, o[1] = 6 * (c[1] - c[0]), a = 2; u - 1 > a; a++) h[a] = 2 * (l[a - 1] + l[a]) - l[a - 1] * l[a - 1] / h[a - 1], o[a] = 6 * (c[a] - c[a - 1]) - l[a - 1] * o[a - 1] / h[a - 1];
		for (r = [], r[u - 1] = r[0] = 0, a = u - 2; a >= 1; a--) r[a] = (o[a] - l[a] * r[a + 1]) / h[a];
		for (e = [], i = [], s = [], n = [], a = 0; u - 1 > a; a++) e[a] = t[a].y, i[a] = -l[a] * r[a + 1] / 6 - l[a] * r[a] / 3 + (t[a + 1].y - t[a].y) / l[a], s[a] = r[a] / 2, n[a] = (r[a + 1] - r[a]) / (6 * l[a]);
	...
	$render_area: function(t, e, i, s, n, a) {
		...
		if (u = this.hp(t, e, i, s, n), h = this.s, l = h.eventRadius || Math.floor(u.cellWidth / 2), e.length) {
			for (d = [], p = h.offset ? i.x + .5 * u.cellWidth : i.x, o = 0; o < e.length; o++) if (c = e[o], b = this.ip(c, i, s, u), v = p + u.cellWidth * o, b) {
				...
				b.out || (d.push([v, g]), a.addRect(c.id, [v - l - i.x, g - l - i.y, v + l - i.x, g + l - i.y], n)), h.yAxis || (r = h.offset || o != e.length - 1 ? "center" : "left", this.canvases[n].renderTextAt(!1, r, v, g - h.labelOffset, h.label(c)))
			}
	...
	$render_stackedArea: function(t, e, i, s, n, a) {
		...
		if (p = this.hp(t, e, i, s, n), l = this.s, f = l.eventRadius || Math.floor(p.cellWidth / 2), e.length) {
			...
			for (c = 0; c < e.length; c++) b = e[c], c ? w += p.cellWidth : (v = m(c, s.y), x.push([w, v])), v = m(c, this.ip(b, i, s, p)), g.push(isNaN(v) && !c ? e[c].$startY || s.y : v), v && (x.push([w, v]), a.addRect(b.id, [w - f - i.x, v - f - i.y, w + f - i.x, v + f - i.y], n), l.yAxis || (o = !l.offset && d ? "left" : "center", this.canvases[n].renderTextAt(!1, o, w, v - l.labelOffset, l.label(b))));
			if (x.push([w, m(c - 1, s.y)]), n) for (c = e.length - 2; c > 0; c--) w -= p.cellWidth, v = e[c].$startY, v && x.push([w, v]);
			for (x.push([x[0][0], x[0][1]]), t.globalAlpha = this.s.alpha.call(this, e[0]), t.fillStyle = this.s.color.call(this, e[0]), t.beginPath(), this.Mo(t, x), t.fill(), c = 0; c < e.length; c++) {
				if (v = g[c], !v) for (c == e.length - 1 && (v = e[c].$startY), u = c + 1; u < e.length; u++) if (g[u]) {
					r = {
						x: i.x,
						y: g[0]
					}, h = {
	...
	zp: function(t, e, i, s, n, a, r, h) {
		var o, l, c, u, d, f, b, p, x, w, v, g, m, y, $, _, C, S, I;
		for (c = this.s, d = c.yAxis.start, f = c.yAxis.end, I = this.Ho(d, f), g = I[0], S = g ? n / g : n / 2, C = I[1], m = -Math.PI / 2, o = l = m, x = [], p = 0, u = 0; u < a.length; u++) _ ? $ = _ : (y = c.value(a[u]), this.Bo && (y = this.Do(y)), $ = (parseFloat(y || 0) - d) * C), w = Math.floor(S * $), y = c.value(u != a.length - 1 ? a[u + 1] : a[0]), this.Bo && (y = this.Do(y)), _ = (parseFloat(y || 0) - d) * C, v = Math.floor(S * _), o = l, l = u != a.length - 1 ? m + e[u] - 1e-4 : m, b = p || this.Qo(o, i, s, w), p = this.Qo(l, i, s, v), x.push(b);
	...
	Bp: function(t, e, i) {
		for (var s, n, a = 0; a < e.length; a++) s = e[a], n = e[a + 1] || e[0], this.zo(t, s.x, s.y, n.x, n.y, this.s.line.color.call(this, i[a]), this.s.line.width)
	},
	yp: function(t, e, i, s, n) {
		...
		if ("undefined" == typeof u.step || "undefined" == typeof u.start || "undefined" == typeof u.end) {
			var d = this.Go();
			c = this.Ao(d.min, d.max), h = c.start, o = c.end, l = c.step, a.end = o, a.start = h
		}
		var f, b, p, x, w, v = [],
			g = 0,
			m = s * l / (o - h);
		...
		for (f = o; f >= h; f -= l) {
			...
			var S, I = -Math.PI / 2,
				E = I;
			for (b = 0; b < t.length; b++) g || y.push(E), S = I + t[b] - 1e-4, this.Dp($, t.length > 2 ? u.lineShape || "line" : "arc", e, i, s - v[v.length - 1], E, S, f, b, n[f]), E = S;
			g++
		}
	...
	Dp: function(t, e, i, s, n, a, r, h, o) {
		...
		var u = this.s.yAxis;
		u.bg && (t.beginPath(), t.moveTo(i, s), "arc" == e ? t.arc(i, s, n, a, r, !1) : (t.lineTo(l.x, l.y), t.lineTo(c.x, c.y)), t.fillStyle = u.bg(h, o), t.moveTo(i, s), t.fill(), t.closePath()), u.lines.call(this, h) && (t.lineWidth = 1, t.beginPath(), "arc" == e ? t.arc(i, s, n, a, r, !1) : (t.moveTo(l.x, l.y), t.lineTo(c.x, c.y)), t.strokeStyle = u.lineColor.call(this, h), t.stroke())
	},
	...
	Fp: function(t, e, i, s, n, a) {
		var r = {};
		return r.totalHeight = s.y - i.y, r.totalWidth = s.x - i.x, this.Hp(r, n.min, n.max, r.totalWidth, "X"), this.Hp(r, a.min, a.max, r.totalHeight, "Y"), r
	},
	Gp: function(t, e, i, s, n, a, r, h, o) {
		var l = this.Ip(n, s, i, a, h, "X"),
	...
	$render_splineArea: function(t, e, i, s, n, a) {
		...
		if (u = this.hp(t, e, i, s, n), c = m.eventRadius || Math.floor(u.cellWidth / 2), o = [], e.length) {
			for (b = i.x, h = 0; h < e.length; h++) w = this.ip(e[h], i, s, u), (w || "0" == w) && (f = h ? u.cellWidth * h - .5 + b : b, o.push({
				x: f,
				y: w,
				index: h
			}), a.addRect(e[h].id, [f - c - i.x, w - c - i.y, f + c - i.x, w + c - i.y], n));
			for (d = this.vp(o), h = 0; h < o.length; h++) if (p = o[h].x, v = o[h].y, h < o.length - 1) {
				for (x = o[h + 1].x, g = o[h + 1].y, l = p; x > l; l++) {
			...
			r = this.s.color.call(this, e[0]), y.length && (y.push([x, s.y]), y.push([y[0][0], s.y])), t.globalAlpha = this.s.alpha.call(this, e[0]), t.fillStyle = r, t.beginPath(), this.Mo(t, y), t.fill(), t.lineWidth = 1, t.globalAlpha = 1, m.border && (t.lineWidth = m.borderWidth || 1, m.borderColor ? t.strokeStyle = m.borderColor.call(this, e[0]) : this.gp(t, r), t.beginPath(), y.splice(y.length - 3), this.Mo(t, y), t.stroke())
		}
	}
}), function() {
	function t(t) {
	...
	function s(t, e) {
		function i(t, i, r, h) {
			var l = t.getCanvas(),
				...
				d = t.Wn,
				f = t.AE;
			if (c && (e.length < s || (e.length - 1) * o > r - i) ? (u.style.left = -c + "px", e.length > 1 && (a(d, c, f), l.clearRect(0, 0, i + c, u.offsetHeight), l.clearRect(r + c, 0, u.offsetWidth, u.offsetHeight))) : (u.style.left = "0px", h || c == o || l.clearRect(i + (e.length - 1) * o - o + c, 0, u.offsetWidth, u.offsetHeight)), d.length > 1 && c && c != o) {
				var b = d.length - 1;
	...
	function l(t, e, i) {
		t.w.t.style.left = e.start.x - i + "px", t.w.t.style.width = e.end.x - e.start.x + i + "px"
	}
	...
	Tp: function(t) {
		for (var e = this.Op(t), i = this.s, s = [], n = [], a = this.bc - 36, r = this.dc - this.s.headerHeight - 10 - (this.s.timepicker || this.ax ? this.s.timepickerHeight : 0), h = i.weekNumber ? 8 : 7, o = 0; h > o; o++) n[o] = Math.ceil(a / (h - o)), a -= n[o];
	...
	render: function() {
		...
		if (this.isVisible(t.id)) {
			...
			if (t.navigation && (r += "<div role='button' tabindex='0' aria-label='" + webix.i18n.aria.navMonth[0] + "' class='webix_cal_prev_button'></div><div role='button' tabindex='0' aria-label='" + webix.i18n.aria.navMonth[1] + "' class='webix_cal_next_button'></div>"), r += "</div>", t.weekHeader && (r += "<div class='webix_cal_header' aria-hidden='true'>" + this.Wp(n) + "</div>"), r += "<div class='webix_cal_body'>" + this.Xp(n, a, i) + "</div>", (this.s.timepicker || this.ax) && (r += "<div class='webix_cal_footer'>", this.s.timepicker && (r += this.Yp(e)), this.ax && (r += this.bx()), r += "</div>"), this.w.innerHTML = r, "time" == this.s.type) {
				var h = this.s.date;
	...
	Xp: function(t, e, i) {
		for (var s = this.s, n = "", a = webix.Date.datePart(webix.Date.copy(i.Rp)), r = s.weekNumber ? 1 : 0, h = webix.Date.getISOWeek(webix.Date.add(a, 2, "day", !0)), o = (this.s.minDate || new Date(1, 1, 1), this.s.maxDate || new Date(9999, 1, 1), 0); o < e.length; o++) {
	...
	aq: {
		...
		"-1": {
			lz: function(t) {
				...
				if (e.blockTime) {
					...
					a.setHours(t);
					for (var r = e.minTime && t == s ? e.minTime[1] : 0, h = e.maxTime && e.maxTime[1] && t == n - 1 ? e.maxTime[1] : 60, o = r; h > o; o += e.minuteStep) if (a.setMinutes(o), !e.blockTime(a)) return !1;
					return !0
				}
			},
			dq: function(t, e) {
			...
			LD: function(t, e, i) {
				...
				return "bottom" === e || "top" === e ? (t.setHours("bottom" === e ? 23 : 0), t.setMinutes("bottom" === e ? 55 : 0), t.setSeconds(0), t.setMilliseconds(0), s = t) : "left" === e || "right" === e ? (n = "right" === e ? a : -a, "left" === e && t.getMinutes() < a && (n = 60 - a), "right" === e && t.getMinutes() >= 60 - a && (n = a - 60), n -= t.getMinutes() % a, s = i.aq[-2].TD(webix.Date.add(t, n, "minute"), e, i)) : "up" === e || "down" === e ? (n = "down" === e ? 1 : -1, "down" === e && 23 === t.getHours() && (n = -23), "up" === e && 0 === t.getHours() && (n = 23), s = this.TD(webix.Date.add(t, n, "hour"), e, i)) : e === !1 && (s = this.TD(t, e, i)), i.selectDate(s, !1), s && (i.cq(s), i.selectDate(s, !1)), "webix_cal_block" + ("left" === e || "right" === e ? "_min" : "")
			},
			TD: function(t, e, i) {
		...
		0: {
			...
			LD: function(t, e, i) {
				var s = t;
				return "pgup" === e || "pgdown" === e ? s = webix.Date.add(t, "pgdown" === e ? 1 : -1, "month") : "bottom" === e ? s = new Date(t.getFullYear(), t.getMonth() + 1, 0) : "top" === e ? s = new Date(t.setDate(1)) : "left" === e || "right" === e ? s = webix.Date.add(t, "right" === e ? 1 : -1, "day") : ("up" === e || "down" === e) && (s = webix.Date.add(t, "down" === e ? 1 : -1, "week")), i.sz(s) || (s = i.TD(t, e)), s && i.selectDate(s, !0), "webix_cal_day"
			}
		},
		1: {
			lz: function(t, e) {
			...
			LD: function(t, e, i) {
				var s = t;
				return "pgup" === e || "pgdown" === e ? s = webix.Date.add(t, "pgdown" === e ? 1 : -1, "year") : "bottom" === e ? s = new Date(t.setMonth(11)) : "top" === e ? s = new Date(t.setMonth(0)) : "left" === e || "right" === e ? s = webix.Date.add(t, "right" === e ? 1 : -1, "month") : ("up" === e || "down" === e) && (s = webix.Date.add(t, "down" === e ? 4 : -4, "month")), i.sz(s) || (s = i.TD(t, e)), s && (i.cq(s), i.selectDate(s, !1)), "webix_cal_block"
			}
		},
		2: {
			lz: function(t, e) {
			...
			LD: function(t, e, i) {
				var s = t;
				return "pgup" === e || "pgdown" === e ? s = webix.Date.add(t, "pgdown" === e ? 10 : -10, "year") : "bottom" === e ? s = new Date(t.setYear(i.gq + 10)) : "top" === e ? s = new Date(t.setYear(i.gq)) : "left" === e || "right" === e ? s = webix.Date.add(t, "right" === e ? 1 : -1, "year") : ("up" === e || "down" === e) && (s = webix.Date.add(t, "down" === e ? 4 : -4, "year")), i.sz(s) || (s = i.TD(t, e)), s && (i.cq(s), i.selectDate(s, !1)), "webix_cal_block"
			}
		}
	},
	mz: function() {
	...
	cq: function(t) {
		...
		if (e = this.s, a = e.weekHeader ? 2 : 1, c = this.aq[this.Np], r = this.w.childNodes, t && (e.date = t), o = e.type, this.Up || (this.hq = this.w.offsetHeight - e.headerHeight, "year" != o && "month" != o ? this.hq -= e.timepickerHeight : this.ax && (this.hq -= 10), this.iq = r[a].offsetWidth, this.Up = 1), this.jq) {
			s = this.hq / 6;
			var f = 6,
				b = this._w || webix.i18n.timeFormat,
				p = b.match(/%([a,A])/);
			for (p && f++, l = parseInt((this.iq - 3) / f, 10), d += "<div class='webix_time_header'>" + this.kq(l, p) + "</div>", d += "<div  class='webix_cal_body' style='height:" + this.hq + "px'>", this.mz(), d += "<div class='webix_hours'>", h = e.date.getHours(), u = webix.Date.copy(e.date), n = 0; 24 > n; n++) {
				if (i = "", p && n % 4 === 0) {
					var x = n ? 12 == n ? webix.i18n.pm[0] : "" : webix.i18n.am[0];
					d += "<div class='webix_cal_block_empty" + i + "' style='" + this.lq(l, s) + "clear:both;'>" + x + "</div>"
				}
				this.aq[-1].lz.call(this, n) ? i += " webix_cal_day_disabled" : h == n && (i += " webix_selected"), u.setHours(n), d += "<div aria-label='" + webix.Date.dateToStr(webix.i18n.aria.hourFormat)(u) + "' role='gridcell' tabindex='" + (h == n ? "0" : "-1") + "' aria-selected='" + (h == n ? "true" : "false") + "' class='webix_cal_block" + i + "' data-value='" + n + "' style='" + this.lq(l, s) + (n % 4 !== 0 || p ? "" : "clear:both;") + "'>" + webix.Date.toFixed(p ? n && 12 != n ? n % 12 : 12 : n) + "</div>"
			}
			...
			for (w[0].innerHTML = c.eq(e.date, this), w[1].setAttribute("aria-label", v[0]), w[2].setAttribute("aria-label", v[1]), s = this.hq / 3, l = this.iq / 4, this.sz(e.date) && (h = 1 == this.Np ? e.date.getMonth() : e.date.getFullYear()), n = 0; 12 > n; n++) {
	...
	setValue: function(t) {
		...
		var e = this.s.value;
		return this.s.value = t, this.$setValue(t, e), t
	},
	uq: null,
	vq: function() {
		...
		function s(t, e, i) {
			...
			if (e) {
				var h = .5 > i ? i * (1 + e) : i + e - i * e,
					o = 2 * i - h;
				s = n(o, h, t + 1 / 3), a = n(o, h, t), r = n(o, h, t - 1 / 3)
			} else s = a = r = i;
			return {
				r: 255 * s,
				g: 255 * a,
	...
	moveSelection: function(t) {
		...
		if (s && (e = this.sq(s)), e || (e = {
			row: 0,
			col: 0
		}), e && ("up" == t || "down" == t ? e.row = e.row + ("up" == t ? -1 : 1) : "right" == t || "left" == t ? e.col = e.col + ("right" == t ? 1 : -1) : "top" == t ? e.row = e.col = 0 : "bottom" == t && (e.row = this.x.lastChild.querySelectorAll(".webix_color_row").length - 1, e.col = this.x.lastChild.childNodes[e.row].childNodes.length - 1), e.row >= 0 && (i = this.x.lastChild.childNodes[e.row].childNodes[e.col]), i)) {
...
}, webix.KeysNavigation, webix.ui.view, webix.EventSystem), webix.protoUI({
	name: "daterange",
	defaults: {
		button: !1,
		icons: !1,
	...
	$init: function(t) {
		...
		this.XD(t, n * r)], t.height = t.height || a.height + (t.icons || t.button ? 35 : 0), t.type = "line", this.$ready.push(this.av), webix.event(this.$view, "keydown", webix.bind(function(t) {
	...
	SD: function(t) {
		return t || (t = {
			...
			start: t
		}), t.end = this.Jp(t.end) || null, t.start = this.Jp(t.start) || null, (t.end && t.end < t.start || !t.start) && (t.end = [t.start, t.start = t.end][0]), t
	},
	Jp: function(t, e) {
	...
	av: function() {
		var t = this.YD = this.getChildViews()[0].getChildViews(),
			e = this;
		this.aE = {};
		for (var i = 0; i < t.length; i++) this.aE[t[i].config.id] = i, t[i].attachEvent("onBeforeDateSelect", function(t) {
			return e.bE(this, t)
	...
	_D: function(t, e) {
		...
		if (0 === this.Np || webix.isUndefined(e)) i.start && !i.end ? i.end = t : (i.start = t, i.end = null);
		else {
			var s = e ? "end" : "start";
			i[s] = t
		}
	...
	bE: function(t, e) {
		if (this.callEvent("onBeforeDateSelect", [e])) {
			...
			if (t.Np < 0) {
				...
				var a = t.getVisibleDate(),
					r = this.fE(t.config.id);
	...
	$compareValue: function(t, e) {
		var i = webix.ui.datepicker.prototype.$compareValue,
	...
	kE: function(t, e) {
		var i = webix.$$(this.s.popup.toString()),
			s = i.getRange();
		return e = webix.copy(s.SD(e)), e.start && (e.start = t(e.start)), e.end && (e.end = t(e.end)), e
	}
	...
	$init: function(t) {
		...
		webix.UE(this.x, webix.env.mouse.down, this.xq, {
			bind: this
		}), webix.event(document.body, webix.env.mouse.up, this.yq, {
			bind: this
		});
		var i = this.zq();
		this.Aq = !1, this.Bq = i, this.Cq = "x" == i ? "width" : "height", "x" == i ? t.height = 0 : t.width = 0, e > 0 ? (this.x.className += " webix_resizer_v" + i, this.x.style.marginRight = "-" + e + "px", "x" == i ? t.width = e : t.height = e, this.$nospace = !0) : this.x.className += " webix_resizer_" + i, this.x.innerHTML = "<div class='webix_resizer_content'></div>", "y" == i && e > 0 && (this.x.style.marginBottom = "-" + (t.height || this.defaults.height) + "px"), this.x.setAttribute("tabindex", "-1"), this.x.setAttribute("aria-grabbed", "false")
	},
	xq: function(t) {
		var e = this.Dq();
	...
	Fq: function(t, e) {
		var i, s, n, a, r;
		t = t || event, i = this.Bq, this.getParentView().x.style.position = "relative", n = webix.html.offset(this.x), a = webix.html.offset(this.getParentView().x), r = n[i] - a[i], s = webix.html.offset(e.$view)[i] - webix.html.offset(this.getParentView().$view)[i], this.Rl = [i, e, r, s], this.Gq = new webix.ui.resizearea({
	...
	Iq: function() {
		var t, e, i, s, n, a, r, h, o, l;
		if (this.Rl) for (t = this.Dq(), n = this.Rl[0], s = this.Gq.Jl - this.Rl[2], o = this.Jq(t, n, s), l = t[0]["$" + this.Cq] + t[1]["$" + this.Cq], r = "y" == n ? ["minHeight", "maxHeight"] : ["minWidth", "maxWidth"], a = 0; 2 > a; a++) {
			e = t[a].s, i = a ? -s : s;
			var c = e[r[0]],
				u = e[r[1]];
			...
			if (this.Lq = null, this.Rl) {
				if (i = this.Rl[0], s = t - this.Rl[2], e = this.Dq(), e[0] && e[1]) {
					...
					for (var n = 0; 2 > n; n++) {
						...
						if (r ? h[2] == h[3] : Math.abs(h[1] - h[0]) < 3) e[n].s[this.Cq] = a[n], e[n].tc && e[n].tc(this.Cq, a[n], r);
						else {
	...
	Mq: function(t, e, i) {
		...
		else for (n = this.Jq(t, e, i), s = 0; 2 > s; s++) n[s] < 0 && (a = n[0] + n[1], n[s] = 1, n[1 - s] = a - 1);
		return n
	}
	...
	webix.patterns = {
		...
		$init: function(e) {
			e.pattern && (this.attachEvent("onKeyPress", function(e, s) {
				if (!s.ctrlKey && !s.altKey) {
					if (e > 105 && 112 > e && (e -= 64), t[e] && 8 !== e && 46 !== e) return void(i[e] || webix.html.preventEvent(s));
		...
		pattern_setter: function(t) {
			...
			return "string" == typeof e && (e = {
				mask: e
			}), e.allow = e.allow || /[A-Za-z0-9]/g, this.bD(e), e
		},
		cD: function() {
			this.config.validate = this.config.validate || webix.bind(function() {
				var t = this.getText(),
					e = t.replace(this.dD, ""),
					i = (t.toString().match(this.eD) || []).join("");
	...
	setValue: function(t) {
		var e = [],
			i = this.getList();
		if (i.unselect(), t && ("string" == typeof t && (t = t.split(this.config.separator)), t[0])) for (var s = 0; s < t.length; s++) i.getItem(t[s]) && (i.exists(t[s]) && i.select(t[s], !0), e.push(this.getItemText(t[s])));
	...
	on_click: {
		webix_multicombo_delete: function(t, e, i) {
			var s;
			return !this.s.readonly && i && (s = i.parentNode.getAttribute("optvalue")) && this.Ay(s), !1
		}
	},
	_E: function() {
		var t = this.getInputNode().value;
	...
	AD: function() {
		var t, e, i, s = [],
			n = this.s.value;
		if (!n) return [];
		for ("string" == typeof n && (n = n.split(this.s.separator)), i = this.getPopup(), t = 0; t < n.length; t++) e = i.getList().getItem(n[t]) || (i.dB ? i.dB[n[t]] : null), e && s.push(e);
		return s
	},
	$setSize: function(t, e) {
		var i = this.s;
	...
	De: function() {
		webix.UE(this.re(), "click", function() {
			...
			bind: this
		}), webix.UE(this.getInputNode(), "keyup", function(t) {
			var e, i, s = this.getInputNode();
			if (t = t || event, !this.s.placeholder || this.s.value || s.value ? (i = e = this.Iy(s.value) + 10, !this.s.tagMode && this.Cy().firstChild && (i = this.lE())) : i = this.Ee(this.s), s.style.width = i + "px", e != this.By ? (this.s.keepText || 13 != t.keyCode ? this.xy = s.value : this.xy = !1, this.Hy = !0, this.By && this.getPopup().show(this.JA()), this.By = e || i, this.Dy()) : this.vA != this.getPopup().$height && this.getPopup().show(this.JA()), s.value.indexOf(this.s.separator) > -1 && this.s.tagMode) {
				var n = s.value.replace(this.s.separator, "");
				if (n) {
					var a = this.getPopup().getItemId(n);
					a ? this.az(a) : this.s.newValues && this.HA(n)
				}
				this.s.keepText ? (this.xy = n, s.value = n, this.bz = !0, this.Hy = !0, this.Dy()) : s.value = ""
			}
		}, {
			bind: this
		}), webix.UE(this.getInputNode(), "keydown", function(t) {
			if (this.bz = !1, this.isVisible()) {
				t = t || event;
				var e = this.Cy().lastChild;
				if (this.vA = this.getPopup().$height, 8 == t.keyCode && e && (!this.getInputNode().value && (new Date).valueOf() - (this.Jy || 0) > 100 ? (this.Hy = !0, this.Ay(e.getAttribute("optvalue"))) : this.Jy = (new Date).valueOf()), 13 == t.keyCode || 9 == t.keyCode) {
					var i = this.getInputNode(),
						s = "",
						n = webix.$$(this.s.suggest),
						a = n.getList();
					a.getSelectedId() || (i.value && (s = n.getSuggestion()), this.s.newValues ? (13 == t.keyCode && (this.bz = !0), this.HA(i.value), this.s.keepText ? this.xy = i.value : i.value = "") : s && (9 == t.keyCode ? (this.Hy = !1, this.xy = "", this.By = 10, i.value = "", this.az(s)) : (this.bz = !0, this.az(s), this.s.keepText ? this.xy = i.value : i.value = ""))), 13 == t.keyCode && (this.bz = !0, this.Hy = !0)
				}
			}
		}, {
			bind: this
	...
	ut: function() {
		...
		if (e = s.length, i.tabMinWidth && a > n / e) return {
			max: parseInt(n / a, 10) || 1
		};
		if (!i.optionWidth) for (t = 0; e > t; t++) s[t].width && (n -= s[t].width + (t || i.type ? 0 : i.tabMargin), e--);
		return {
			width: e ? n / e : i.tabMinWidth
		}
	},
	Je: function() {
		var t = this.s;
	...
	defaults: {
		...
		template: function(t, e) {
			...
			if (e.OC = l = e.yC(t.options), l.length) {
				...
				o = t.tabOffset;
				var v = !1;
				for (n = 0; n < l.length && !v; n++) d && d.max ? (d.max == n + 1 && (v = !0), i = e.we - 2 * t.tabOffset - (!t.type && d.max > 1 ? t.tabMargin * (d.max - 1) : 0), u = (i - t.tabMoreWidth) / d.max) : u = d.width, u = l[n].width || t.optionWidth || u, o += u + (n && !t.type ? t.tabMargin : 0), t.tabMargin > 0 && n && !t.type && (s += "<div class='webix_tab_filler' style='width:" + t.tabMargin + "px;'></div>"), s += e.vt(l[n], u), v && (s += '<div role="button" tabindex="0" aria-label="' + webix.i18n.aria.showTabs + '" class="webix_tab_more_icon" style="width:' + t.tabMoreWidth + 'px;">' + t.moreTemplate(t, e) + "</div>", o += t.tabMoreWidth);
			...
			return r = "", h = c && !t.type ? "height:" + (e.dc - c) + "px" : "", t.topOffset && !t.type && (r += "<div class='webix_before_all_tabs' style='width:100%;height:" + t.topOffset + "px'></div>"), r += "<div style='" + h + "' role='tablist' class='webix_all_tabs " + (t.type ? "webixtype_" + t.type : "") + "'>" + s + "</div>", t.bottomOffset && !t.type && (r += "<div class='webix_after_all_tabs' style='width:100%;height:" + t.bottomOffset + "px'></div>"), r
		}
	},
	Ie: function() {
	...
	vt: function(t, e) {
		var i, s = "",
			n = this.config;
		if (t.id == n.value && (s = " webix_selected"), t.css && (s += " " + t.css), e = t.width || e, i = '<div class="webix_item_tab' + s + '" button_id="' + t.id + '" role="tab" aria-selected="' + (t.id == n.value ? "true" : "false") + '" tabindex="' + (t.id == n.value ? "0" : "-1") + '" style="width:' + e + 'px;">', this.lr) {
			var a = this.dc - 2 * n.inputPadding - 2,
				r = this.dc - 2,
				h = webix.extend({
					cheight: a,
					aheight: r
	...
	select: function(t, e) {
		var i;
		if ("object" != typeof t && (t = [t]), e) for (i = 0; i < t.length; i++) this.uF[t[i]] = !0;
		else for (i = 0; i < t.length; i++) delete this.uF[t[i]];
		...
		pc: function() {
			...
			for (var a = 0; a < n.length; a++) {
				...
				if (h) {
					...
					this.elements[h] = r, r.getValue = i, r.setValue = s, r.Ce = !c, r.s = {
						defaultValue: r.getValue()
					}
				}
			}
	...
	webix.protoUI({
		name: "google-map",
		$init: function(t) {
			this.$view.innerHTML = "<div class='webix_map_content' style='width:100%;height:100%'></div>", this.w = this.$view.firstChild, this.iF = webix.promise.defer(), this.data.provideApi(this, !0), this.$ready.push(this.render)
		},
		getMap: function(t) {
		...
		center_setter: function(e) {
			return this.eo && this.eo.setCenter(new t.maps.LatLng(e[0], e[1])), e
		},
		...
		layerType_setter: function(t) {
			return "heatmap" == t && (this.config.libraries = "visualization"), this.nE[t] && (webix.extend(this, this.nE[t], !0), this.data.attachEvent("onStoreUpdated", webix.bind(this.drawData, this))), t
		},
		defaults: {
			zoom: 5,
			center: [39.5, -98.5],
		...
		nE: {
			marker: {
				...
				oE: function(e) {
					...
					return this.h(n), this.callEvent("onItemRender", [e]), n
				},
				h: function(t) {
					var e = this;
				...
				if (!n.Hr && !n.Er) {
					...
					if (h && (this.s.store.uf && this.s.store.uf(n.data), this.callEvent("onBefore" + r, [a, n]))) {
						...
						h.$proxy ? h.save ? h.save(this.config.master, n, this, l) : e.push(n) : ("insert" == r && delete n.data.id, o ? h(n.id, n.operation, n.data).then(function(t) {
							...
							l.error("", null, t)
						}) : (n.data[this.s.operationName] = r, this.G(h, n.data, this.s.mode, r, l))), this.callEvent("onAfterDataSend", [n])
					}
				}
			}
	...
	Jr: function(t, e, i, s, n, a) {
		...
		if (h.Hr = !1, t) {
			if (this.callEvent("onBeforeSaveError", [e, s, n, a])) return h.Er = !0, this.s.undoOnError && r.s.undo && r.undo(e), void this.callEvent("onAfterSaveError", [e, s, n, a])
		...
		}), this.s.undoOnError && r.s.undo && r.removeUndo(i || e), this.callEvent("onAfterSave", [n, e, a]), this.callEvent("onAfter" + s, [n, e, a])
	},
	...
	xr: function(t, e, i, s) {
		if (this.callEvent("onBeforeSync", [t, e, i, s]), -1 === s) this.processResult(t, i, {});
			...
			if (n.$proxy && n.result) n.result(t, this.s.master, this, e, i, s);
				...
				var a;
				e && (a = i.json(), e && "undefined" == typeof a && (a = {
					status: "error"
...
}, webix.Settings, webix.EventSystem, webix.ValidateData), function() {
	var t = {};
	...
	init: function(t, e, i) {
		...
		for (var h = 0; h < s.length; h++) {
			var o;
			o = this.Or(s[h], a), o.$scope = i, r = this.Pr(o, s[h], a, e)
		}
		return r
	},
	parse: function(t, e) {
			...
			if (1 == u.nodeType) {
				...
				if ("column" == n) {
					...
					"head" == p.view ? i.head = p.rows ? p.rows[0] : p.template : "body" == p.view ? this.Mr(i.view, "addView") ? o.push({
						...
						header: p.header || ""
					}) : i.body = p.rows ? 1 == p.rows.length ? p.rows[0] : {
						rows: p.rows
					} : p.value : o.push(p)
				} else if (c) {
					l = 1;
	...
	Vr: function(t, e, i) {
		...
		for (var n = t.childNodes, a = 0; a < n.length; a++) {
			var r = null;
			r = 1 == n[a].nodeType && "config" == n[a].tagName.toLowerCase() && n[a].attributes.length ? this.Vr(n[a], s, i) : n[a].innerHTML, s.stack && r && e[s.stack].push(r)
		}
		return e
	},
	Tr: function(t, e) {
	...
	DD: function(t, e) {
		this.s.navigation.items && "tab" === e.target.getAttribute("role") && this.PD(t, e), webix.ui.baseview.prototype.DD.call(this, t, e)
	},
	getChildViews: function() {
		return [this.Ft]
	},
	...
	Mi: function() {
		...
		}, this)), webix.ui.each(this.Ft, function(t) {
	...
	scrollTo: function(t, e) {
		webix.Touch && webix.animate.isSupported() && this.s.animate ? webix.Touch.Nf(this.w, t, e, this.s.scrollSpeed || "100ms") : (this.w.style.marginLeft = t + "px", this.w.style.marginTop = e + "px")
	},
	navigation_setter: function(t) {
		return this.E(t, {
			type: "corner",
	...
	flash: {
		$render: function(t) {
			...
			var e = this.s;
			e.swfId = e.swfId || "webix_swf_" + webix.uid(), this.re().innerHTML += "<div class='webix_upload_flash'><div id='" + e.swfId + "'></div></div>", this._r = this.re().lastChild, swfobject.embedSWF(webix.codebase + "/legacy/uploader.swf", e.swfId, "100%", "100%", "9", null, {
				uploaderId: e.id,
				ID: e.swfId,
	...
	html5: {
		...
		send: function(t) {
			...
			n.status = "transfer";
			var a = new FormData;
			if (n.folder) for (var s = 0; s < n.folder.length; s++) a.append(this.config.inputName + s, n.folder[s], n.folder[s].webkitRelativePath);
		...
		is: function(t) {
			...
			if (e) {
				var i = null;
				e.xhr.status < 400 && (i = webix.DataDriver[this.s.datatype || "json"].toObject(e.xhr.responseText)), i && "error" != i.status ? this.js(t, i) : (e.status = "error", delete e.percent, this.files.updateItem(t), this.callEvent("onFileUploadError", [e, i])), delete e.xhr
			}
		},
		stopUpload: function(t) {
	...
	$init: function() {
		webix.env.touch ? this.attachEvent("onTouchStart", webix.bind(this.ss, this)) : webix.UE(this.x, "mousedown", webix.bind(this.ss, this)), webix.UE(this.$view, "keydown", webix.bind(this.tE, this))
	},
	$skin: function() {
	...
	tE: function(t) {
		...
		if (e > 32 && 41 > e) {
			...
			if (this.My = a ? parseInt(a[1], 10) : -1, a && (s = i.value[this.My]), s = s < i.min ? i.min : s > i.max ? i.max : s, 35 === e) s = i.min;
	...
	xs: function(t) {
		...
		var e = this.s.value;
		webix.isArray(e) && (e = webix.copy(e)), this.s.value = this.$x, this.setValue(e), this.rs(this.My).focus(), this.My = -1
	},
	ws: function(t) {
	...
	oe: function() {
		var t, e, i, s, n, a, r;
		e = this.rs(0), i = this.rs(1), t = this.s, webix.isArray(t.value) || this.define("value", t.value), e && (r = this.Ee(t), a = e.parentNode, a.style.width = r + "px", s = this.Ly(r, 0), n = this.Ly(r, 1), e.style.left = 10 + s - 8 + "px", i.style.left = 10 + n - 8 + "px", a.firstChild.style.width = r - 22 + "px", a.childNodes[1].style.width = n - s + "px", a.childNodes[1].style.left = s + 12 + "px", this.s.title && (e.parentNode.previousSibling.innerHTML = this.s.title(this.s, this)))
	},
	sE: function() {
	...
	ys: function(t) {
		...
		if (h != o || e.value[0] != e.min && e.value[0] != e.max) if (this.My >= 0) r = this.My;
	...
	result: function(t, e, i, s, n) {
		for (var a = 0; a < n.length; a++) i.processResult({
			id: n[a].id,
			status: n[a].operation
		}, {}, {})
	}
}, webix.proxy.cache = {
	init: function() {
	...
	yB: function(t) {
		var e = this.config.master ? webix.$$(this.config.master) : this,
			i = e.getValue(),
			s = e.s.value;
	...
	addSection: function() {
		...
		return webix.$$(i).attachEvent("onChange", this.yB), this.eu.push(i), i
	},
	...
	_x: function(t) {
		...
		if (t || (this.ay = [], this.$xy = {}, n = this.$width - 2 * this.type.padding, this.$xy[0] = {
			...
			for (!t && c && (this.y.style.width = c + 2 * this.type.padding + "px"), e = 0; e < r.length; e++) i = r[e], this.data.branch[i] && this.getItem(i).open ? a += this._x(i) : o && ("list" != o.$type ? this.ay.push(this.$xy[i].top + this.$xy[i].height) : e == r.length - 1 && this.ay.push(this.$xy[i].top + this.$xy[i].height));
	...
	sC: function(t, e) {
		var i, s = t.s.name;
		if (s) i = "$value" == s ? e : e[s], t.setValues ? t.setValues(i) : t.setValue ? t.setValue(i) : t.parse && (t.openAll && (i = webix.copy(i)), t.parse(i));
		else {
			var n = t.q;
	...
	render: function(t, e, i) {
		if (t && "update" === i) {
			var e = this.getItem(t),
				s = this.getIndexById(t);
			return void this.sC(this.q[s], e)
		}
		for (var n = this.nc = [], a = this.data.order, r = this.cC.length, h = 0; h < a.length; h++) if (r) for (var o = 0; r > o; o++) n.push(webix.copy(this.cC[o]));
		else n.push(this.getItem(a[h]));
		if (n.length || n.push({}), this.reconstruct(), r) for (var h = 0; h < a.length; h++) for (var l = this.getItem(a[h]), o = 0; r > o; o++) {
			var c = this.q[h * r + o];
			this.sC(c, l)
		}
	}
	...
	Td: function(t) {
		var e, i, s, n, a, r = 0,
			h = 0,
			o = {};
		this.$view.style.position = "fixed", s = window.innerWidth || document.documentElement.offsetWidth, n = window.innerHeight || document.documentElement.offsetHeight, e = this.MA[0] || s, i = this.MA[2] || n, a = this.s.position, "top" == a ? e = s : "right" == a ? (i = n, r = s - e) : "bottom" == a ? (e = s, h = n - i) : i = n, o = {
			left: r,
			top: h,
			width: e,
			height: i,
	...
	var t = webix.CustomScroll = {
		...
		ku: function(e) {
			if (t.gu = this, clearTimeout(this.tu), !this.ou && !t.qu) {
				...
				}, i.ec = i.dx > i.px && -1 != this.mu.indexOf("x"), i.cc = i.dy > i.py && -1 != this.mu.indexOf("y");
				this.ou = i, i.ec && (i.uu = t.Om(this, "x", i.dx, i.px, "width", "height"), i.vu = i.px - i.uu.offsetWidth - 4, i.wu = i.dx - i.px, t.trackBar && (i.wA = t.xA(this, "x"))), i.cc && (i.xu = t.Om(this, "y", i.dy, i.py, "height", "width"), i.yu = i.py - i.xu.offsetHeight - 4, i.zu = i.dy - i.py, t.trackBar && (i.yA = t.xA(this, "y"))), t.sk(this)
			}
		},
		xA: function(t, e) {
			var i = webix.html.create("DIV", {
		...
		sk: function(t, e, i) {
			...
			if (s && (s.uu || s.xu)) {
				var n = t.iu,
					...
					l = n ? webix.$$(n).jk : o,
					c = n ? 0 : l;
				s.uu && (s.uu.style.bottom = 1 - c + "px", s.uu.style.left = Math.round(s.vu * r / (s.dx - s.px)) + h + 1 + "px", s.wA && (s.wA.style.bottom = 1 - c + "px", s.wA.style.left = h + "px")), s.xu && (s.xu.style.right = 0 - h + "px", s.xu.style.top = Math.round(s.yu * l / (s.dy - s.py)) + c + 1 + "px", s.yA && (s.yA.style.right = 0 - h + "px", s.yA.style.top = c + "px"))
			}
		},
		wE: function(e) {
	...
	function e(t, e) {
		...
		if (r) {
			if (!r.length) {
				var o = [];
				for (var l in r) o.push(webix.extend({
					id: l
				}, webix.extend({}, r[l])));
				r = o
			}
		...
		if (e.id && i.push({
			...
			for (var u = e.flatTree.id, d = [].concat(e.flatTree.columns), f = [], b = !! e.flatTree.fill, p = 1; p <= d.length; p++) d[p - 1].template = function(t, e) {
				return function(e) {
					return e.$level == t ? f[t] = e[u] : b && t < e.$level ? f[t] : ""
				}
			}(p);
			for (var x = 0, p = r.length - 1; p >= 0; p--) r[p].id === u && (x = p);
			r = [].concat(r.slice(0, x)).concat(d).concat(r.slice(x + 1))
		}
		for (var w = 0; w < r.length; w++) {
			var v = r[w],
				l = v.id;
			if (!v.noExport) {
				a && t.Aj[l] && (v = webix.extend(webix.extend({}, v), t.Aj[l]));
				var g = {
					id: v.id,
					template: (h ? null : v.template) ||
					function(t, e) {
						return function(i) {
							return e.format ? e.format(i[t]) : i[t]
						}
					}(l, v),
					width: (v.width || 200) * ("excel" === e.gB ? 8.43 / 70 : 1),
					header: v.header !== !1 ? v.header || l : ""
				};
				"string" == typeof g.header ? g.header = [{
					text: g.header
				}] : g.header = webix.copy(g.header);
				for (var p = 0; p < g.header.length; p++) g.header[p] = g.header[p] ? g.header[p].contentId ? "" : g.header[p].text : "";
				if (s = Math.max(s, g.header.length), t.s.footer) {
					var m = v.footer || "";
					m = "string" == typeof m ? [{
						text: m
					}] : webix.copy(m);
					for (var p = 0; p < m.length; p++) m[p] ? m[p] = m[p].contentId ? t.getHeaderContent(m[p].contentId).getValue() : m[p].text : m[p] = "";
		...
		for (var p = 0; p < i.length; p++) {
			for (var y = s - i[p].header.length, $ = 0; y > $; $++) i[p].header.push("");
			if (t.s.footer) {
				y = n - i[p].footer.length;
				for (var $ = 0; y > $; $++) i[p].footer.push("")
			}
		}
		return i
	}
	function i(t, e, i) {
		var s, n, a = !! e.filterHTML,
			r = /<[^>]*>/gi,
			h = [];
		if (e.header !== !1 && i.length) for (var o = 0; o < i[0].header.length; o++) {
			n = [];
			for (var l = 0; l < i.length; l++) s = "", i[l].header[o] && (s = i[l].header[o], a && (s = i[l].header[o] = s.replace(r, ""))), n.push(s);
			"excel" === e.gB && h.push(n), "csv" === e.gB && (n.map(function(t) {
				t.replace(/<[^>]*>/gi, "")
			}), h.push(n))
		}
		...
		if (t.data.each(function(s) {
			if (s) {
				for (var n = [], o = 0; o < i.length; o++) {
					...
					d || 0 === d || (d = ""), a && "string" == typeof d && (c && (d = d.replace(/<div class=.webix_tree_none.><\/div>/, u)), d = d.replace(r, "")), "string" == typeof d && "csv" === e.gB && (d = d.trim()), "string" != typeof d || "excel" !== e.gB && "csv" !== e.gB || (d = d.replace(/<br\s*\/?>/gm, "\n")), n.push(d)
				}
				h.push(n)
			}
	...
	function r(t, e, i) {
		...
		}, h = 0; h != t.length; ++h) for (var o = 0; o != t[h].length; ++o) {
	...
	function o(t, e, i, s) {
		...
		for (var x = 0; x < e.length; x++) for (var w = d.tr({}), v = 0; v < e[x].length; v++) w.td(e[x][v]);
		if (l) for (var g = webix.extend(i.footer, {
			...
			height: 27,
			lineHeight: 1.2
		}), r = 0; l > r; r++) for (var m = d.tr(g), p = 0; p < t.length; p++) m.td(t[p].footer[r].toString());
		if (i.docFooter !== !1) {
			var y = h.footer();
			y.text({
				color: 6710886,
		...
		if (i.docHeaderImage) {
			...
			var _ = h.header({
				...
				C = webix.extend(i.docHeaderImage, {
					align: "right"
				});
	...
	webix.toPNG = function(t, e) {
		...
		var a = webix.$$(s);
		n = n || {}, a.$exportView && (a = a.$exportView(n)), n.gB = "csv";
		var r = e(a, n),
			h = i(a, n, r),
			o = t(h, r),
	...
	webix.toPDF = function(t, s) {
		...
		return webix.require(webix.cdn + "/extras/pdfjs.js", function() {
			...
			s.hB ? o(r, h, s, c) : pdfjs.load(webix.cdn + "/extras/" + s.fontName + ".ttf", function(t, e) {
				if (t) throw t;
	...
	$init: function(t) {
		...
		this.uz = this.$view, this.gt = this.$view.appendChild(e), this.Zn = this.gt.appendChild(i), this.$pdfDoc = null, this.$pageNum = 0, this.$numPages = 0, this.vz = !1, this.wz = null, this.xz = this.Zn.getContext("2d"), this.zz = .1, this.Az = t.scaleDelta || 1.1, this.Bz = t.minScale || .25, this.Cz = t.maxScale || 10, this.Dz = 1.25, this.Ez = 40, this.Fz = 10, this.$ready.push(this.Hz)
	},
	...
	Hz: function() {
		if (delete this.s.datatype, this.attachEvent("onScaleChange", function(t, e) {
			...
			var e = t.targetTouches;
			2 === e.length && this.xE !== !1 && (webix.html.preventEvent(t), Math.abs(e[0].pageY - e[1].pageY) > this.xE ? this.zoomIn() : this.zoomOut(), this.xE = !1)
	...
	yE: function(t) {
		...
		} else PDFJS.getDocument({
			data: t.data
		}).then(webix.bind(function(t) {
			this.clear(), this.$pdfDoc = t, this.$numPages = this.$pdfDoc.numPages, this.$pageNum = 1, this.Jz(this.$pageNum).then(webix.bind(function() {
	...
	Nz: function(t, e) {
		this.s.scale = t, this.renderPage(this.$pageNum), this.callEvent("onScaleChange", [t, e])
	},
	download: function() {
		if (this.$pdfDoc) {
	...
	clear: function() {
		this.$pdfDoc && (this.xz.clearRect(0, 0, this.Zn.width, this.Zn.height), this.gt.style.height = this.gt.style.width = this.Zn.width = this.Zn.height = 0, this.s.scale = "auto", this.$pageNum = this.$numPages = 0, this.$pdfDoc.transport.startCleanup(), this.$pdfDoc.destroy(), this.$pdfDoc = null, this.s.toolbar && webix.$$(this.s.toolbar) && webix.$$(this.s.toolbar).reset())
	}
}, webix.EventSystem, webix.AtomDataLoader, webix.ui.view), webix.protoUI({
	name: "pdfbar",
	reset: function() {
	...
	$init: function(t) {
		this.$view.className += " pdf_bar", t.cols = [{
			...
			template: webix.i18n.PDFviewer.of + " #limit#",
			width: 70,
			data: {
				limit: 0
			},
	...
	Oz: function() {
		...
		t.clearAll(), t.parse([{
			id: "auto",
			value: webix.i18n.PDFviewer.automaticZoom
		}, {
			id: "page-actual",
			value: webix.i18n.PDFviewer.actualSize
		}, {
			id: "page-fit",
			value: webix.i18n.PDFviewer.pageFit
		}, {
			id: "page-width",
			value: webix.i18n.PDFviewer.pageWidth
		}, {
			id: "page-height",
			value: webix.i18n.PDFviewer.pageHeight
		}, {
			id: "0.5",
			value: "50%"
		}, {
		...
		}), this.getChildViews()[6].define("width", e + 20), this.getChildViews()[6].resize()
	},
	he: function(t) {
	...
	sheetToArray: function(t, e) {
		...
		if (t["!ref"]) {
			...
			for (n = o.s.r; n <= o.e.r; n++) {
				var l = [];
				for (a = o.s.c; a <= o.e.c; a++) if (r = XLS.utils.encode_cell({
					r: n,
					c: a
				}), h = t[r]) {
					var c = "";
					c = e.math && h.f ? "=" == h.f.charAt(0) ? h.f : "=" + h.f : "n" == h.t && h.w && h.v != h.w ? h.w : h.v, l.push(c)
				} else l.push("");
				i.push(l)
			}
		}
		if (t["!merges"]) for (var u = t["!merges"], d = 0; d < u.length; d++) {
			var f = u[d].s,
				b = u[d].e,
				p = e.rows ? e.rows[0] : 0;
			s.push([f.r + p, f.c, b.c - f.c + 1, b.r - f.r + 1])
		}
		return {
			data: i,
			spans: s,
	...
	_x: function(t) {
		...
		if (t || (t = this.config.branch || 0, this.v = "", this.$values = {}, this.$xy = {}, this.$xy[t] = {
			...
			if (!this.data.branch[t.id] && (n = 1 * this.config.value.call(this, t), !isNaN(n) && n)) for (this.$values[t.id] = n, a += n; e;) this.$values[e] || (this.$values[e] = 0), this.$values[e] += n, e = this.getParentId(e)
	...
	iB: function() {
		...
		}, this.attachEvent("onBeforeAreaAdd", this.oB), webix.UE(this.Vf, "mousedown", this.pB, {
			bind: this
		})
	},
	ql: !0,
	uC: function(t) {
		...
		for (var i in e) {
			var s = e[i];
			if (this.getColumnIndex(s.start.column) < 0) if (s.start.column == s.end.column) this.removeSelectArea(s.name);
			else {
	...
	gC: function(t, e, i) {
		var s, n, a, r, h, o, l, c;
		if (e) {
			s = this.getColumnIndex(e.start.column), n = this.getColumnIndex(e.end.column), a = this.getIndexById(e.start.row), r = this.getIndexById(e.end.row), h = this.getColumnIndex(t.column), o = this.getIndexById(t.row), l = this.getIndexById(e.init.row), c = this.getColumnIndex(e.init.column), s > h || "left" == i ? "left" === i && n > c ? n-- : s = h : (h >= n || "right" == i) && ("right" == i && c > s ? s++ : n = h), a > o || "up" == i ? "up" == i && r > l ? r-- : a = o : (o > r || "down" == i) && ("down" == i && l > a ? a++ : r = o);
			var u = {
	...
	addSelectArea: function(t, e, i, s, n, a) {
		...
		return n && (u.css = n), (a || a === !1) && (u.handle = a), this.kB[s] ? this.vC(s, u.start, u.end) : (u.handle = !0, u.name = s, u.init = u.start, void(this.callEvent("onBeforeAreaAdd", [u]) && (this.qB = s, i || this.removeSelectArea(), this.kB[u.name] = u, this.el.push(u), this.refreshSelectArea(), this.callEvent("onAfterAreaAdd", [u]), this.callEvent("onSelectChange", []))))
	},
	PC: function() {
		var t = webix.html.create("DIV");
	...
	BD: function(t, e, i) {
		...
		return a = Math.max(e[0], l), r = Math.min(this.Fj ? e[1] - 1 : e[1], c), {
			r0: s,
			r1: n,
			c0: a,
			c1: r
	...
	SC: function(t, e, i, s) {
		var n = this.ug(this.getIdByIndex(t), this.columnId(e)),
			a = this.ug(this.getIdByIndex(i), this.columnId(s));
		return [n, a]
	},
	sB: function(t, e, i, s, n, a) {
		...
		var c = function(t, e, i, a, r) {
				...
				for (h in b) {
					u = e.top + l, "bottom" == h && (u = i.top + i.height), d = e.left, "right" == h && (d = i.left + i.width), f = "top" == h || "bottom" == h, t.appendChild(webix.html.create("DIV", {
	...
	oB: function(t) {
		...
		var e, i, s, n, a, r, h, o, l, c, u, d, f, b = !1,
			p = t.start,
			x = t.end;
		for (c = a = this.getIndexById(p.row), u = s = this.getColumnIndex(p.column), d = r = this.getIndexById(x.row), f = n = this.getColumnIndex(x.column), e = a; r >= e; e++) for (i = s; n >= i; i++) h = this.getSpan(this.getIdByIndex(e), this.columnId(i)), h && (o = this.getIndexById(h[0]), l = this.getColumnIndex(h[1]), c > o && (c = o, b = !0), u > l && (b = !0, u = l), o + h[3] - 1 > d && (b = !0, d = o + h[3] - 1), l + h[2] - 1 > f && (b = !0, f = l + h[2] - 1));
		b && (t.start = {
	...
	AB: function() {
		...
		}), this.x.appendChild(this.GB), this.x.appendChild(this.CB), this.x.appendChild(this.FB), this.pF(), webix.UE(this.FB, webix.env.mouse.down, this.IB, {
			bind: this
		}), webix.UE(this.GB, webix.env.mouse.down, this.IB, {
			bind: this
		}), webix.UE(this.CB, webix.env.mouse.down, this.IB, {
			bind: this
		}), webix.UE(webix.toNode(this.FB), "keydown", this.zE, {
			bind: this
		}), webix.UE(webix.toNode(this.GB), "keydown", this.zE, {
			bind: this
		}), this.JB && (this.s.range = this.BB(this.JB)), this.KB(), this.callEvent("onAfterRangeChange", [this.JB]), void this.data.attachEvent("onStoreUpdated", webix.bind(this.KB, this)))
	},
	$setSize: function(t, e) {
	...
	IB: function(t) {
		...
		webix.html.addCss(this.x, "webix_noselect webix_wresize_cursor"), this.GE(), this.NB = webix.event(document.body, webix.env.mouse.move, this.OB, {
			bind: this
	...
	GE: function() {
		webix.h[this.NB] && (webix.eventRemove(this.NB), webix.eventRemove(this.PB))
	},
	OB: function(t) {
		if (this.LB) {
			...
			e = {
				...
				on: {
					onAfterRender: function() {
						webix.UE(t.getInputNode(), "input", function() {
	...
	$setSize: function(t, e) {
		webix.ui.view.prototype.$setSize.call(this, t, e) && (this.QE(this.config.value), this.Qf(this.config.value))
	},
	QE: function() {
		var t = this.$view.querySelector(".webix_gage_curves"),
			...
			i = this.config.scale,
			s = this.$width;
		t.setAttribute("r", s / i), t.setAttribute("strokeDasharray", Math.round(Math.PI * s / i)), t.style.r = s / i, t.style.strokeDasharray = Math.round(Math.PI * s / i), e.setAttribute("style", "width: " + Math.round(s / i * 2) + "px;"), this.PE.setAttribute("style", "height: " + (s / i + 20) + "px;"), this.NE.setAttribute("r", s / i), this.NE.setAttribute("style", "stroke-dasharray: " + Math.round(this.gradientLength * Math.PI * s / i) + ", 1900;"), this.RE(t.style.r)
	},
	sF: function(t) {
	...
	RE: function(t) {
		...
		(0 === a || 180 === a) && (this.PE.style.paddingTop = "3px"), this.OE.style.transformOrigin = e / 2 - (.5 + i) + "px 0 0", this.OE.setAttribute("y1", "0"), this.OE.setAttribute("x1", Math.round(e * (.5 + i))), this.OE.setAttribute("y2", 0), this.OE.setAttribute("x2", Math.round(e * (.5 + i / 2) + parseInt(t)))
	},
	Qf: function(t) {
	...
	$init: function(t) {
		t && (t.layout && "x" !== t.layout || t.height || (t.height = t.scale === !1 ? 60 : 90), "y" !== t.layout || t.width || (t.width = t.scale === !1 ? 60 : 97))
	},
	...
	CF: function(t) {
		...
		this.FF += i, Math.abs(this.FF - this.s.value) < Math.abs(i) && (this.FF = this.s.value), this.FF != this.s.value ? this.GF = requestAnimationFrame(this.CF.bind(this)) : (cancelAnimationFrame(this.GF), this.GF = null), this.HF.setAttribute("width", Math.floor(this.FF * this.JF))
	},
	Qf: function(t, e) {
	...
	LF: function(t, e, i) {
		var s = "http://www.w3.org/2000/svg",
			n = document.createElementNS(s, t);
		return e && this.KF(n, e, i), n
	},
	SF: function(t) {
		var e = this.LF(t[0], t[1], t[2]),
			i = t[3];
	...
	MF: function() {
		var t = "d" + webix.uid(),
			e = this.LF("svg", ["class"], ["webix_bullet_graph_svg"]),
			i = this.LF("g"),
			s = this.LF("g"),
			...
			o = "y" == this.s.layout ? "50%" : this.IF - 10,
			l = "y" == this.s.layout ? 11 : 17,
			...
			f = this.SF(["filter", ["id", "x", "y", "width", "height"],
				[t, "0", "-150%", "110%", "400%"],
				[
					["feOffset", ["in", "dx", "dy"],
						["SourceAlpha", 0, 0]
		...
		if (this.s.scale) {
			for (var x = this.s.minRange; x <= this.s.maxRange; x += this.s.scale.step) {
				var w = this.s.labelHeight || this.s.labelWidth,
					o = Math.floor(w + x * this.JF - (x ? .1 : -1)),
					v = p ? (this.$width - this.s.barWidth) / 2 + -10 : o,
					g = p ? this.NF - o + w + 44 : this.s.barWidth + 28,
					m = p ? -13 : this.s.barWidth + 3,
					y = p ? "end" : "middle",
					$ = this.LF("tspan", ["x", "y", "text-anchor"], [v, g, y]),
					_ = this.LF("line", ["x1", "y1", "x2", "y2", "stroke-width"], [o, m, o, m + 10, 1]);
				b.innerHTML = this.s.scale.template({
					value: x
				}), $.appendChild(b.childNodes[0]), d.appendChild($), r.appendChild(_)
			}
			i.appendChild(r), e.appendChild(d)
		}
		for (var x = 0; x < this.s.bands.length; x++) {
			var C = this.s.bands[x],
				S = this.LF("path"),
				n = this.sF(C.value) * this.JF;
			S.setAttribute("d", "M " + this.IF + ",0 l " + n + ",0 l 0," + this.s.barWidth + " l -" + n + ",0 z"), S.setAttribute("fill", C.color), s.appendChild(S)
		}
		if (e.appendChild(i), "y" === this.s.layout) {
			var I = this.s.scale ? this.$width / 2 - 10 : 0,
				E = this.$height + this.IF - 28;
			i.setAttribute("transform", "translate(" + I + ", " + E + ") rotate(270)"), h.setAttribute("text-anchor", "middle"), h.childNodes[0].setAttribute("x", "55%"), h.childNodes[1].setAttribute("x", "55%"), d.setAttribute("text-anchor", "right")
		}
	...
	ME: function(t) {
		if (t) {
			...
			this.IF = this.s.label ? i : 7, this.TF = Math.floor((this.s.barWidth - this.s.stroke) / 2), this.NF = t - this.IF - 30, this.JF = this.NF / (this.s.maxRange - this.s.minRange);
			var s = this.MF();
